...
import { c as zodToJsonSchema, d as _OverloadYield, l as _awaitAsyncGenerator, u as _wrapAsyncGenerator } from "../@lovable.dev/mcp-js.mjs";
import { d as _objectSpread2, f as init_objectSpread2, m as init_defineProperty, p as _defineProperty, u as _objectWithoutProperties } from "../@dnd-kit/core.mjs";
...
//#region \0@oxc-project+runtime@0.133.0/helpers/esm/asyncIterator.js
init_defineProperty();
init_objectSpread2();
...
//#endregion
//#region node_modules/@mendable/firecrawl-js/dist/chunk-DBXKWZGC.js
...
var platform_default = _objectSpread2(_objectSpread2({}, utils_exports), node_default);
//#endregion
...
var readBlob = /* @__PURE__ */ function() {
	var _ref = _wrapAsyncGenerator(function* (blob) {
		if (blob.stream) yield* _asyncGeneratorDelegate(_asyncIterator(blob.stream()));
		else if (blob.arrayBuffer) yield yield _awaitAsyncGenerator(blob.arrayBuffer());
		else if (blob[asyncIterator]) yield* _asyncGeneratorDelegate(_asyncIterator(blob[asyncIterator]()));
		else yield blob;
	});
...
var FormDataPart = class {
	...
	encode() {
		...
		return _wrapAsyncGenerator(function* () {
			...
			else yield* _asyncGeneratorDelegate(_asyncIterator(readBlob(value)));
			yield CRLF_BYTES;
		})();
	}
...
function getTunnelingAgent(agentOptions, userHttpsAgent) {
	...
	agent = new import_dist.default(userHttpsAgent && userHttpsAgent.options ? _objectSpread2(_objectSpread2({}, userHttpsAgent.options), agentOptions) : agentOptions);
...
init_objectSpread2();
var headersToObject = (thing) => thing instanceof AxiosHeaders$1 ? _objectSpread2({}, thing) : thing;
/**
...
function getVersion() {
	try {
		...
		return (pkg === null || pkg === void 0 ? void 0 : pkg.version) || "3.x.x";
	} catch (_unused) {
		return "3.x.x";
	}
}
var HttpClient = class {
	constructor(options) {
		var _options$maxRetries, _options$backoffFacto, _options$timeoutMs;
		_defineProperty(this, "instance", void 0);
		_defineProperty(this, "apiKey", void 0);
		_defineProperty(this, "apiUrl", void 0);
		_defineProperty(this, "maxRetries", void 0);
		_defineProperty(this, "backoffFactor", void 0);
		this.apiKey = options.apiKey;
		this.apiUrl = options.apiUrl.replace(/\/$/, "");
		this.maxRetries = (_options$maxRetries = options.maxRetries) !== null && _options$maxRetries !== void 0 ? _options$maxRetries : 3;
		this.backoffFactor = (_options$backoffFacto = options.backoffFactor) !== null && _options$backoffFacto !== void 0 ? _options$backoffFacto : .5;
		this.instance = axios.create({
			baseURL: this.apiUrl,
			timeout: (_options$timeoutMs = options.timeoutMs) !== null && _options$timeoutMs !== void 0 ? _options$timeoutMs : 3e5,
			headers: _objectSpread2({}, this.apiKey ? { Authorization: `Bearer ${this.apiKey}` } : {}),
	...
	async request(config) {
		...
		config.headers = _objectSpread2({}, config.headers || {});
		let lastError;
		for (let attempt = 0; attempt < _this.maxRetries; attempt++) try {
			const cfg = _objectSpread2({}, config);
			...
			if (!isFormDataBody && cfg.data != null && typeof cfg.data === "object" && !Array.isArray(cfg.data) && cfg.method && [
				...
				var _cfg$data;
				const data = (_cfg$data = cfg.data) !== null && _cfg$data !== void 0 ? _cfg$data : {};
				cfg.data = _objectSpread2(_objectSpread2({}, data), {}, { origin: typeof data.origin === "string" && data.origin.includes("mcp") ? data.origin : `js-sdk@${version}` });
			}
			if (isFormDataBody) {
				cfg.headers = _objectSpread2({}, cfg.headers || {});
				delete cfg.headers["Content-Type"];
				delete cfg.headers["content-type"];
			}
			const res = await _this.instance.request(cfg);
			if (res.status === 502 && attempt < _this.maxRetries - 1) {
				await _this.sleep(_this.backoffFactor * Math.pow(2, attempt));
				continue;
			}
			return res;
		} catch (err) {
			var _err$response;
			lastError = err;
			if ((err === null || err === void 0 || (_err$response = err.response) === null || _err$response === void 0 ? void 0 : _err$response.status) === 502 && attempt < _this.maxRetries - 1) {
				await _this.sleep(_this.backoffFactor * Math.pow(2, attempt));
				continue;
			}
			throw err;
		}
		throw (_lastError = lastError) !== null && _lastError !== void 0 ? _lastError : /* @__PURE__ */ new Error("Unexpected HTTP client error");
	}
	sleep(seconds) {
...
var JobTimeoutError = class extends SdkError {
	constructor(jobId, timeoutSeconds, jobType = "batch") {
		...
		_defineProperty(this, "timeoutSeconds", void 0);
...
function tryZodV4Conversion(schema) {
	...
	try {
		...
		const zodModule = (_schema$constructor = schema.constructor) === null || _schema$constructor === void 0 || (_schema$constructor = _schema$constructor.prototype) === null || _schema$constructor === void 0 ? void 0 : _schema$constructor.constructor;
...
function throwForBadResponse(resp, action) {
	...
	throw new SdkError((body === null || body === void 0 ? void 0 : body.error) || (body === null || body === void 0 ? void 0 : body.message) || `Request failed (${status}) while trying to ${action}`, status, void 0, body === null || body === void 0 ? void 0 : body.details);
}
function normalizeAxiosError(err, action) {
	var _err$response2, _err$response3, _body$details;
	const status = (_err$response2 = err.response) === null || _err$response2 === void 0 ? void 0 : _err$response2.status;
	const body = (_err$response3 = err.response) === null || _err$response3 === void 0 ? void 0 : _err$response3.data;
	throw new SdkError((body === null || body === void 0 ? void 0 : body.error) || err.message || `Request failed${status ? ` (${status})` : ""} while trying to ${action}`, status, (body === null || body === void 0 ? void 0 : body.code) || err.code, (_body$details = body === null || body === void 0 ? void 0 : body.details) !== null && _body$details !== void 0 ? _body$details : body);
}
function isRetryableError(err) {
	...
	if ((err === null || err === void 0 ? void 0 : err.isAxiosError) && !err.response) return true;
	if ((err === null || err === void 0 ? void 0 : err.code) === "ECONNABORTED" || (err === null || err === void 0 || (_err$message = err.message) === null || _err$message === void 0 ? void 0 : _err$message.includes("timeout"))) return true;
	return true;
}
async function scrape(http, url, options) {
	...
	try {
		var _res$data;
		const res = await http.post("/v2/scrape", payload, typeof (options === null || options === void 0 ? void 0 : options.timeout) === "number" ? { timeoutMs: options.timeout + 5e3 } : {});
		if (res.status !== 200 || !((_res$data = res.data) === null || _res$data === void 0 ? void 0 : _res$data.success)) throwForBadResponse(res, "scrape");
		return res.data.data || {};
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "scrape");
		throw err;
	}
}
async function interact(http, jobId, args) {
	...
	const hasCode = (args === null || args === void 0 ? void 0 : args.code) && args.code.trim();
	const hasPrompt = (args === null || args === void 0 ? void 0 : args.prompt) && args.prompt.trim();
	...
	body.language = (_args$language = args.language) !== null && _args$language !== void 0 ? _args$language : "node";
	...
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "interact with scrape browser");
		throw err;
	}
}
async function stopInteraction(http, jobId) {
	...
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "stop interaction");
		throw err;
	}
}
function toUploadBlob(input, contentType) {
	...
	if (typeof input === "string") return new Blob([input], { type: contentType !== null && contentType !== void 0 ? contentType : "text/plain; charset=utf-8" });
...
async function parse(http, file, options) {
	...
	const version = getVersion();
	const normalizedOptions = _objectSpread2(_objectSpread2({}, options !== null && options !== void 0 ? options : {}), {}, { origin: typeof (options === null || options === void 0 ? void 0 : options.origin) === "string" && options.origin.includes("mcp") ? options.origin : (_options$origin = options === null || options === void 0 ? void 0 : options.origin) !== null && _options$origin !== void 0 ? _options$origin : `js-sdk@${version}` });
	...
	try {
		...
		if (res.status !== 200 || !((_res$data2 = res.data) === null || _res$data2 === void 0 ? void 0 : _res$data2.success)) throwForBadResponse(res, "parse");
		return res.data.data || {};
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "parse");
		throw err;
	}
}
function prepareSearchPayload(req) {
	var _req$includeDomains, _req$excludeDomains;
	...
	if (((_req$includeDomains = req.includeDomains) === null || _req$includeDomains === void 0 ? void 0 : _req$includeDomains.length) && ((_req$excludeDomains = req.excludeDomains) === null || _req$excludeDomains === void 0 ? void 0 : _req$excludeDomains.length)) throw new Error("includeDomains and excludeDomains cannot both be specified");
	...
	if (req.threatProtection != null) payload.threatProtection = req.threatProtection;
...
async function search(http, request) {
	...
	try {
		...
		if (res.status !== 200 || !((_res$data3 = res.data) === null || _res$data3 === void 0 ? void 0 : _res$data3.success)) throwForBadResponse(res, "search");
		...
		if (data.developer) out.developer = transformArray(data.developer);
		Object.defineProperty(out, "data", {
			get() {
				var _out$web, _out$news, _out$images, _out$developer;
				const parts = [];
				if ((_out$web = out.web) === null || _out$web === void 0 ? void 0 : _out$web.length) parts.push(`.web (${out.web.length} results)`);
				if ((_out$news = out.news) === null || _out$news === void 0 ? void 0 : _out$news.length) parts.push(`.news (${out.news.length} results)`);
				if ((_out$images = out.images) === null || _out$images === void 0 ? void 0 : _out$images.length) parts.push(`.images (${out.images.length} results)`);
				if ((_out$developer = out.developer) === null || _out$developer === void 0 ? void 0 : _out$developer.length) parts.push(`.developer (${out.developer.length} results)`);
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "search");
		throw err;
	}
}
function prepareMapPayload(url, options) {
	...
	if (options) {
		...
		if (options.threatProtection != null) payload.threatProtection = options.threatProtection;
		if (options.auditMetadata != null) payload.auditMetadata = options.auditMetadata;
	}
	return payload;
}
async function map(http, url, options) {
	...
	try {
		var _res$data4;
		const res = await http.post("/v2/map", payload, typeof (options === null || options === void 0 ? void 0 : options.timeout) === "number" ? { timeoutMs: options.timeout + 5e3 } : {});
		if (res.status !== 200 || !((_res$data4 = res.data) === null || _res$data4 === void 0 ? void 0 : _res$data4.success)) throwForBadResponse(res, "map");
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "map");
		throw err;
	}
}
...
async function feedback(http, request) {
	...
	try {
		...
		if (res.status !== 200 || !((_res$data5 = res.data) === null || _res$data5 === void 0 ? void 0 : _res$data5.success)) throwForBadResponse(res, "feedback");
		return res.data;
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "feedback");
		throw err;
	}
}
async function searchFeedback(http, jobId, request) {
	...
	try {
		...
		if (res.status !== 200 || !((_res$data6 = res.data) === null || _res$data6 === void 0 ? void 0 : _res$data6.success)) throwForBadResponse(res, "searchFeedback");
		return res.data;
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "searchFeedback");
		throw err;
	}
}
async function fetchAllPages(http, nextUrl, initial, pagination) {
	var _pagination$maxPages, _pagination$maxResult, _pagination$maxWaitTi;
	const docs = initial.slice();
	let current = nextUrl;
	let pageCount = 0;
	const maxPages = (_pagination$maxPages = pagination === null || pagination === void 0 ? void 0 : pagination.maxPages) !== null && _pagination$maxPages !== void 0 ? _pagination$maxPages : void 0;
	const maxResults = (_pagination$maxResult = pagination === null || pagination === void 0 ? void 0 : pagination.maxResults) !== null && _pagination$maxResult !== void 0 ? _pagination$maxResult : void 0;
	const maxWaitTime = (_pagination$maxWaitTi = pagination === null || pagination === void 0 ? void 0 : pagination.maxWaitTime) !== null && _pagination$maxWaitTi !== void 0 ? _pagination$maxWaitTi : void 0;
	const started = Date.now();
	while (current) {
		var _payload$data, _ref, _payload$next, _payload$data2;
		...
		if (!(payload === null || payload === void 0 ? void 0 : payload.success)) break;
		const pageData = Array.isArray(payload.data) ? payload.data : ((_payload$data = payload.data) === null || _payload$data === void 0 ? void 0 : _payload$data.pages) || [];
		for (const d of pageData) {
			if (maxResults != null && docs.length >= maxResults) break;
			docs.push(d);
		}
		if (maxResults != null && docs.length >= maxResults) break;
		current = (_ref = (_payload$next = payload.next) !== null && _payload$next !== void 0 ? _payload$next : Array.isArray(payload.data) ? null : (_payload$data2 = payload.data) === null || _payload$data2 === void 0 ? void 0 : _payload$data2.next) !== null && _ref !== void 0 ? _ref : null;
		pageCount += 1;
	}
	return docs;
}
...
async function startCrawl(http, request) {
	...
	try {
		...
		if (res.status !== 200 || !((_res$data7 = res.data) === null || _res$data7 === void 0 ? void 0 : _res$data7.success)) throwForBadResponse(res, "start crawl");
		return {
			id: res.data.id,
			url: res.data.url
		};
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "start crawl");
		throw err;
	}
}
async function getCrawlStatus(http, jobId, pagination) {
	try {
		var _res$data8, _pagination$autoPagin, _body$completed2, _body$total2;
		const res = await http.get(`/v2/crawl/${jobId}`);
		if (res.status !== 200 || !((_res$data8 = res.data) === null || _res$data8 === void 0 ? void 0 : _res$data8.success)) throwForBadResponse(res, "get crawl status");
		const body = res.data;
		const initialDocs = body.data || [];
		if (!((_pagination$autoPagin = pagination === null || pagination === void 0 ? void 0 : pagination.autoPaginate) !== null && _pagination$autoPagin !== void 0 ? _pagination$autoPagin : true) || !body.next) {
			var _body$completed, _body$total, _body$next;
			return {
				id: jobId,
				status: body.status,
				completed: (_body$completed = body.completed) !== null && _body$completed !== void 0 ? _body$completed : 0,
				total: (_body$total = body.total) !== null && _body$total !== void 0 ? _body$total : 0,
				creditsUsed: body.creditsUsed,
				expiresAt: body.expiresAt,
				next: (_body$next = body.next) !== null && _body$next !== void 0 ? _body$next : null,
				data: initialDocs
			};
		}
		...
		return {
			id: jobId,
			status: body.status,
			completed: (_body$completed2 = body.completed) !== null && _body$completed2 !== void 0 ? _body$completed2 : 0,
			total: (_body$total2 = body.total) !== null && _body$total2 !== void 0 ? _body$total2 : 0,
			creditsUsed: body.creditsUsed,
			expiresAt: body.expiresAt,
			next: null,
			data: aggregated
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get crawl status");
		throw err;
	}
}
async function cancelCrawl(http, jobId) {
	try {
		...
		return ((_res$data9 = res.data) === null || _res$data9 === void 0 ? void 0 : _res$data9.status) === "cancelled";
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "cancel crawl");
		throw err;
	}
}
...
async function getCrawlErrors(http, crawlId) {
	try {
		var _res$data$data, _res$data10;
		const res = await http.get(`/v2/crawl/${crawlId}/errors`);
		if (res.status !== 200) throwForBadResponse(res, "get crawl errors");
		const payload = (_res$data$data = (_res$data10 = res.data) === null || _res$data10 === void 0 ? void 0 : _res$data10.data) !== null && _res$data$data !== void 0 ? _res$data$data : res.data;
		return {
			errors: payload.errors || [],
			robotsBlocked: payload.robotsBlocked || []
		};
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get crawl errors");
		throw err;
	}
}
async function getActiveCrawls(http) {
	try {
		var _res$data11, _res$data12;
		const res = await http.get(`/v2/crawl/active`);
		if (res.status !== 200 || !((_res$data11 = res.data) === null || _res$data11 === void 0 ? void 0 : _res$data11.success)) throwForBadResponse(res, "get active crawls");
		return {
			success: true,
			crawls: (((_res$data12 = res.data) === null || _res$data12 === void 0 ? void 0 : _res$data12.crawls) || []).map((c) => {
				var _c$teamId, _c$options;
				return {
					id: c.id,
					teamId: (_c$teamId = c.teamId) !== null && _c$teamId !== void 0 ? _c$teamId : c.team_id,
					url: c.url,
					options: (_c$options = c.options) !== null && _c$options !== void 0 ? _c$options : null
				};
			})
		};
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get active crawls");
		throw err;
	}
}
async function crawlParamsPreview(http, url, prompt) {
	...
	try {
		...
		if (res.status !== 200 || !((_res$data13 = res.data) === null || _res$data13 === void 0 ? void 0 : _res$data13.success)) throwForBadResponse(res, "crawl params preview");
		const data = res.data.data || {};
		if (res.data.warning) data.warning = res.data.warning;
		return data;
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "crawl params preview");
		throw err;
	}
}
async function startBatchScrape(http, urls, { options, webhook, appendToId, ignoreInvalidURLs, maxConcurrency, zeroDataRetention, idempotencyKey, integration, origin } = {}) {
	...
	try {
		...
		if (res.status !== 200 || !((_res$data14 = res.data) === null || _res$data14 === void 0 ? void 0 : _res$data14.success)) throwForBadResponse(res, "start batch scrape");
		return {
			id: res.data.id,
			url: res.data.url,
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "start batch scrape");
		throw err;
	}
}
async function getBatchScrapeStatus(http, jobId, pagination) {
	try {
		var _res$data15, _pagination$autoPagin2, _body$completed4, _body$total4;
		const res = await http.get(`/v2/batch/scrape/${jobId}`);
		if (res.status !== 200 || !((_res$data15 = res.data) === null || _res$data15 === void 0 ? void 0 : _res$data15.success)) throwForBadResponse(res, "get batch scrape status");
		const body = res.data;
		const initialDocs = body.data || [];
		if (!((_pagination$autoPagin2 = pagination === null || pagination === void 0 ? void 0 : pagination.autoPaginate) !== null && _pagination$autoPagin2 !== void 0 ? _pagination$autoPagin2 : true) || !body.next) {
			var _body$completed3, _body$total3, _body$next2;
			return {
				id: jobId,
				status: body.status,
				completed: (_body$completed3 = body.completed) !== null && _body$completed3 !== void 0 ? _body$completed3 : 0,
				total: (_body$total3 = body.total) !== null && _body$total3 !== void 0 ? _body$total3 : 0,
				creditsUsed: body.creditsUsed,
				expiresAt: body.expiresAt,
				next: (_body$next2 = body.next) !== null && _body$next2 !== void 0 ? _body$next2 : null,
				data: initialDocs
			};
		}
		const aggregated = await fetchAllPages(http, body.next, initialDocs, pagination);
		return {
			id: jobId,
			status: body.status,
			completed: (_body$completed4 = body.completed) !== null && _body$completed4 !== void 0 ? _body$completed4 : 0,
			total: (_body$total4 = body.total) !== null && _body$total4 !== void 0 ? _body$total4 : 0,
			creditsUsed: body.creditsUsed,
			expiresAt: body.expiresAt,
			next: null,
			data: aggregated
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get batch scrape status");
		throw err;
	}
}
async function cancelBatchScrape(http, jobId) {
	try {
		...
		return ((_res$data16 = res.data) === null || _res$data16 === void 0 ? void 0 : _res$data16.status) === "cancelled";
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "cancel batch scrape");
		throw err;
	}
}
async function getBatchScrapeErrors(http, jobId) {
	try {
		var _res$data$data2, _res$data17;
		const res = await http.get(`/v2/batch/scrape/${jobId}/errors`);
		if (res.status !== 200) throwForBadResponse(res, "get batch scrape errors");
		const payload = (_res$data$data2 = (_res$data17 = res.data) === null || _res$data17 === void 0 ? void 0 : _res$data17.data) !== null && _res$data$data2 !== void 0 ? _res$data$data2 : res.data;
		return {
			errors: payload.errors || [],
			robotsBlocked: payload.robotsBlocked || []
		};
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get batch scrape errors");
		throw err;
	}
}
...
async function batchScrape(http, urls, opts = {}) {
	var _opts$pollInterval;
	return waitForBatchCompletion(http, (await startBatchScrape(http, urls, opts)).id, (_opts$pollInterval = opts.pollInterval) !== null && _opts$pollInterval !== void 0 ? _opts$pollInterval : 2, opts.timeout);
}
function prepareExtractPayload(args) {
	...
	if (args.threatProtection != null) body.threatProtection = args.threatProtection;
...
async function startExtract(http, args) {
	...
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "extract");
		throw err;
	}
}
async function getExtractStatus(http, jobId) {
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "extract status");
		throw err;
	}
}
...
async function extract(http, args) {
	var _args$pollInterval;
	const started = await startExtract(http, args);
	const jobId = started.id;
	if (!jobId) return started;
	return waitExtract(http, jobId, (_args$pollInterval = args.pollInterval) !== null && _args$pollInterval !== void 0 ? _args$pollInterval : 2, args.timeout);
}
function prepareAgentPayload(args) {
	...
	if (args.webhook != null) body.webhook = args.webhook;
	if (args.threatProtection != null) body.threatProtection = args.threatProtection;
	if (args.auditMetadata != null) body.auditMetadata = args.auditMetadata;
	return body;
}
async function startAgent(http, args) {
	...
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "agent");
		throw err;
	}
}
async function getAgentStatus(http, jobId) {
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "agent status");
		throw err;
	}
}
...
async function agent(http, args) {
	var _args$pollInterval2;
	const started = await startAgent(http, args);
	const jobId = started.id;
	if (!jobId) return started;
	return waitAgent(http, jobId, (_args$pollInterval2 = args.pollInterval) !== null && _args$pollInterval2 !== void 0 ? _args$pollInterval2 : 2, args.timeout);
}
async function cancelAgent(http, jobId) {
	try {
		...
		return ((_res$data18 = res.data) === null || _res$data18 === void 0 ? void 0 : _res$data18.success) === true;
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "cancel agent");
		throw err;
	}
}
async function browser(http, args = {}) {
	...
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "create browser session");
		throw err;
	}
}
async function browserExecute(http, sessionId, args) {
	...
	const body = {
		code: args.code,
		language: (_args$language2 = args.language) !== null && _args$language2 !== void 0 ? _args$language2 : "bash"
	};
	if (args.timeout != null) body.timeout = args.timeout;
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "execute browser code");
		throw err;
	}
}
async function deleteBrowser(http, sessionId) {
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "delete browser session");
		throw err;
	}
}
async function listBrowsers(http, args = {}) {
	...
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "list browser sessions");
		throw err;
	}
}
async function getConcurrency(http) {
	try {
		var _res$data19, _d$maxConcurrency;
		const res = await http.get("/v2/concurrency-check");
		if (res.status !== 200 || !((_res$data19 = res.data) === null || _res$data19 === void 0 ? void 0 : _res$data19.success)) throwForBadResponse(res, "get concurrency");
		const d = res.data.data || res.data;
		return {
			concurrency: d.concurrency,
			maxConcurrency: (_d$maxConcurrency = d.maxConcurrency) !== null && _d$maxConcurrency !== void 0 ? _d$maxConcurrency : d.max_concurrency
		};
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get concurrency");
		throw err;
	}
}
async function getCreditUsage(http) {
	try {
		var _res$data20, _ref2, _d$remainingCredits, _d$planCredits, _ref3, _d$billingPeriodStart, _ref4, _d$billingPeriodEnd;
		const res = await http.get("/v2/team/credit-usage");
		if (res.status !== 200 || !((_res$data20 = res.data) === null || _res$data20 === void 0 ? void 0 : _res$data20.success)) throwForBadResponse(res, "get credit usage");
		const d = res.data.data || res.data;
		return {
			remainingCredits: (_ref2 = (_d$remainingCredits = d.remainingCredits) !== null && _d$remainingCredits !== void 0 ? _d$remainingCredits : d.remaining_credits) !== null && _ref2 !== void 0 ? _ref2 : 0,
			planCredits: (_d$planCredits = d.planCredits) !== null && _d$planCredits !== void 0 ? _d$planCredits : d.plan_credits,
			billingPeriodStart: (_ref3 = (_d$billingPeriodStart = d.billingPeriodStart) !== null && _d$billingPeriodStart !== void 0 ? _d$billingPeriodStart : d.billing_period_start) !== null && _ref3 !== void 0 ? _ref3 : null,
			billingPeriodEnd: (_ref4 = (_d$billingPeriodEnd = d.billingPeriodEnd) !== null && _d$billingPeriodEnd !== void 0 ? _d$billingPeriodEnd : d.billing_period_end) !== null && _ref4 !== void 0 ? _ref4 : null
		};
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get credit usage");
		throw err;
	}
}
async function getTokenUsage(http) {
	try {
		var _res$data21, _ref5, _d$remainingTokens, _d$planTokens, _ref6, _d$billingPeriodStart2, _ref7, _d$billingPeriodEnd2;
		const res = await http.get("/v2/team/token-usage");
		if (res.status !== 200 || !((_res$data21 = res.data) === null || _res$data21 === void 0 ? void 0 : _res$data21.success)) throwForBadResponse(res, "get token usage");
		const d = res.data.data || res.data;
		return {
			remainingTokens: (_ref5 = (_d$remainingTokens = d.remainingTokens) !== null && _d$remainingTokens !== void 0 ? _d$remainingTokens : d.remaining_tokens) !== null && _ref5 !== void 0 ? _ref5 : 0,
			planTokens: (_d$planTokens = d.planTokens) !== null && _d$planTokens !== void 0 ? _d$planTokens : d.plan_tokens,
			billingPeriodStart: (_ref6 = (_d$billingPeriodStart2 = d.billingPeriodStart) !== null && _d$billingPeriodStart2 !== void 0 ? _d$billingPeriodStart2 : d.billing_period_start) !== null && _ref6 !== void 0 ? _ref6 : null,
			billingPeriodEnd: (_ref7 = (_d$billingPeriodEnd2 = d.billingPeriodEnd) !== null && _d$billingPeriodEnd2 !== void 0 ? _d$billingPeriodEnd2 : d.billing_period_end) !== null && _ref7 !== void 0 ? _ref7 : null
		};
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get token usage");
		throw err;
	}
}
async function getQueueStatus(http) {
	try {
		...
		if (res.status !== 200 || !((_res$data22 = res.data) === null || _res$data22 === void 0 ? void 0 : _res$data22.success)) throwForBadResponse(res, "get queue status");
		return res.data;
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get queue status");
		throw err;
	}
}
async function getCreditUsageHistorical(http, byApiKey) {
	try {
		...
		if (res.status !== 200 || !((_res$data23 = res.data) === null || _res$data23 === void 0 ? void 0 : _res$data23.success)) throwForBadResponse(res, "get credit usage historical");
		return res.data;
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get credit usage historical");
		throw err;
	}
}
async function getTokenUsageHistorical(http, byApiKey) {
	try {
		...
		if (res.status !== 200 || !((_res$data24 = res.data) === null || _res$data24 === void 0 ? void 0 : _res$data24.success)) throwForBadResponse(res, "get token usage historical");
		return res.data;
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get token usage historical");
		throw err;
	}
}
...
function normalizeResearchError(err, action) {
	if (err === null || err === void 0 ? void 0 : err.isAxiosError) {
		var _err$response4, _err$response5;
		const status = (_err$response4 = err.response) === null || _err$response4 === void 0 ? void 0 : _err$response4.status;
		const body = (_err$response5 = err.response) === null || _err$response5 === void 0 ? void 0 : _err$response5.data;
...
var ResearchClient = class {
	...
	*/
	async searchPapers(query, options = {}) {
		...
		try {
			const res = await _this2.http.get(withQuery(`${BASE}/papers`, params));
	...
	async getPaper(id, options = {}) {
		...
		appendParam(params, "k", options.k);
		try {
			const res = await _this3.http.get(withQuery(`${BASE}/papers/${encodeURIComponent(id)}`, params));
	...
	*/
	async similarPapers(id, options) {
		var _this4 = this;
		if (!id || !id.trim()) throw new Error("id cannot be empty");
		if (!(options === null || options === void 0 ? void 0 : options.intent) || !options.intent.trim()) throw new Error("intent cannot be empty");
		if (options.k != null && options.k <= 0) throw new Error("k must be positive");
		const params = new URLSearchParams();
		...
		try {
			const res = await _this4.http.get(withQuery(`${BASE}/papers/${encodeURIComponent(id)}/similar`, params));
	...
	*/
	async searchGithub(query, options = {}) {
		...
		appendParam(params, "k", options.k);
		try {
			const res = await _this5.http.get(withQuery(`${BASE}/github`, params));
...
function dataOrThrow(res, action) {
	var _res$data25;
	if (res.status !== 200 || !((_res$data25 = res.data) === null || _res$data25 === void 0 ? void 0 : _res$data25.success) || res.data.data == null) throwForBadResponse(res, action);
	return res.data.data;
}
async function createMonitor(http, request) {
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "create monitor");
		throw err;
	}
}
async function listMonitors(http, options) {
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "list monitors");
		throw err;
	}
}
async function getMonitor(http, monitorId) {
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get monitor");
		throw err;
	}
}
async function updateMonitor(http, monitorId, request) {
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "update monitor");
		throw err;
	}
}
async function deleteMonitor(http, monitorId) {
	try {
		...
		if (res.status !== 200 || !((_res$data26 = res.data) === null || _res$data26 === void 0 ? void 0 : _res$data26.success)) throwForBadResponse(res, "delete monitor");
		return true;
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "delete monitor");
		throw err;
	}
}
async function runMonitor(http, monitorId) {
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "run monitor");
		throw err;
	}
}
async function listMonitorChecks(http, monitorId, options) {
	try {
		...
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "list monitor checks");
		throw err;
	}
}
async function getMonitorCheck(http, monitorId, checkId, options) {
	try {
		var _ref9, _res$data$next, _res$data27, _options$autoPaginate;
		const _ref8 = options !== null && options !== void 0 ? options : {}, { autoPaginate: _autoPaginate, maxPages: _maxPages, maxResults: _maxResults, maxWaitTime: _maxWaitTime } = _ref8, query = _objectWithoutProperties(_ref8, _excluded);
		...
		const next = (_ref9 = (_res$data$next = (_res$data27 = res.data) === null || _res$data27 === void 0 ? void 0 : _res$data27.next) !== null && _res$data$next !== void 0 ? _res$data$next : detail.next) !== null && _ref9 !== void 0 ? _ref9 : null;
		if (!((_options$autoPaginate = options === null || options === void 0 ? void 0 : options.autoPaginate) !== null && _options$autoPaginate !== void 0 ? _options$autoPaginate : true) || !next) return _objectSpread2(_objectSpread2({}, detail), {}, { next });
		return _objectSpread2(_objectSpread2({}, detail), {}, {
			pages: await fetchAllPages(http, next, detail.pages || [], options),
			next: null
		});
	} catch (err) {
		if (err === null || err === void 0 ? void 0 : err.isAxiosError) return normalizeAxiosError(err, "get monitor check");
		throw err;
	}
}
...
var isNodeRuntime = () => {
	...
	return typeof process !== "undefined" && !!((_process$versions = process.versions) === null || _process$versions === void 0 ? void 0 : _process$versions.node);
};
var cachedWebSocket;
var loadPromise;
var loadNodeWebSocket = async () => {
	...
	try {
		var _undici$WebSocket, _undici$default;
		const undici = await import("undici");
		const ctor = (_undici$WebSocket = undici.WebSocket) !== null && _undici$WebSocket !== void 0 ? _undici$WebSocket : (_undici$default = undici.default) === null || _undici$default === void 0 ? void 0 : _undici$default.WebSocket;
...
var ensureUtf8String = (data) => {
	...
	const convertView = (view) => {
		...
		return decoder === null || decoder === void 0 ? void 0 : decoder.decode(view);
	};
...
var Watcher = class extends EventEmitter {
	constructor(http, jobId, opts = {}) {
		var _opts$kind, _opts$pollInterval2;
		super();
		_defineProperty(this, "http", void 0);
		_defineProperty(this, "jobId", void 0);
		...
		_defineProperty(this, "emittedDocumentKeys", /* @__PURE__ */ new Set());
		this.http = http;
		this.jobId = jobId;
		this.kind = (_opts$kind = opts.kind) !== null && _opts$kind !== void 0 ? _opts$kind : "crawl";
		this.pollInterval = (_opts$pollInterval2 = opts.pollInterval) !== null && _opts$pollInterval2 !== void 0 ? _opts$pollInterval2 : 2;
	...
	async start() {
		...
		return new Promise((resolve, reject) => {
			...
			const cleanup = () => {
				_this6.removeListener("done", onDone);
				_this6.removeListener("error", onError);
			};
			_this6.on("done", onDone);
			_this6.on("error", onError);
			(async () => {
				try {
					const url = _this6.buildWsUrl();
					const wsCtor = await getWebSocketCtor();
					if (!wsCtor) {
						_this6.pollLoop();
						return;
					}
					_this6.ws = new wsCtor(url, _this6.http.getApiKey());
					if (_this6.ws && "binaryType" in _this6.ws) _this6.ws.binaryType = "arraybuffer";
					if (_this6.ws) _this6.attachWsHandlers(_this6.ws);
				} catch (err) {
					_this6.pollLoop();
				}
			})();
	...
	documentKey(doc) {
		if (doc && typeof doc === "object") {
			var _ref10, _doc$id;
			const explicitId = (_ref10 = (_doc$id = doc.id) !== null && _doc$id !== void 0 ? _doc$id : doc.docId) !== null && _ref10 !== void 0 ? _ref10 : doc.url;
	...
	emitDocuments(docs) {
		for (const doc of docs) {
			...
			this.emit("document", _objectSpread2(_objectSpread2({}, doc), {}, { id: this.jobId }));
		}
	}
	emitSnapshot(payload) {
		var _payload$completed, _payload$total, _payload$next2, _payload$completed2, _payload$total2, _payload$next3;
		const status = payload.status;
		const data = payload.data || [];
		const snap = this.kind === "crawl" ? {
			id: this.jobId,
			status,
			completed: (_payload$completed = payload.completed) !== null && _payload$completed !== void 0 ? _payload$completed : 0,
			total: (_payload$total = payload.total) !== null && _payload$total !== void 0 ? _payload$total : 0,
			creditsUsed: payload.creditsUsed,
			expiresAt: payload.expiresAt,
			next: (_payload$next2 = payload.next) !== null && _payload$next2 !== void 0 ? _payload$next2 : null,
			data
		} : {
			id: this.jobId,
			status,
			completed: (_payload$completed2 = payload.completed) !== null && _payload$completed2 !== void 0 ? _payload$completed2 : 0,
			total: (_payload$total2 = payload.total) !== null && _payload$total2 !== void 0 ? _payload$total2 : 0,
			creditsUsed: payload.creditsUsed,
			expiresAt: payload.expiresAt,
			next: (_payload$next3 = payload.next) !== null && _payload$next3 !== void 0 ? _payload$next3 : null,
			data
		};
		...
		if ([
			...
			var _payload$total3, _payload$completed3;
			this.emit("done", {
				status,
				data,
				id: this.jobId,
				total: (_payload$total3 = payload.total) !== null && _payload$total3 !== void 0 ? _payload$total3 : 0,
				completed: (_payload$completed3 = payload.completed) !== null && _payload$completed3 !== void 0 ? _payload$completed3 : 0,
				creditsUsed: payload.creditsUsed
			});
			this.close();
		}
	...
	async pollLoop() {
		...
		const timeoutMs = _this7.timeout ? _this7.timeout * 1e3 : void 0;
		while (!_this7.closed) {
			try {
				const snap = _this7.kind === "crawl" ? await getCrawlStatus(_this7.http, _this7.jobId) : await getBatchScrapeStatus(_this7.http, _this7.jobId);
				_this7.emitDocuments(snap.data || []);
				_this7.emit("snapshot", snap);
				if ([
					"completed",
					"failed",
					"cancelled"
				].includes(snap.status)) {
					var _snap$total, _snap$completed;
					_this7.emit("done", {
						status: snap.status,
						data: snap.data,
						id: _this7.jobId,
						total: (_snap$total = snap.total) !== null && _snap$total !== void 0 ? _snap$total : 0,
						completed: (_snap$completed = snap.completed) !== null && _snap$completed !== void 0 ? _snap$completed : 0,
			...
			await new Promise((r) => setTimeout(r, Math.max(1e3, _this7.pollInterval * 1e3)));
		}
	}
	close() {
		this.closed = true;
...
var FirecrawlClient = class {
	...
	*/
	constructor(options = {}) {
		var _ref11, _opts$apiKey, _ref12, _opts$apiUrl;
		_defineProperty(this, "http", void 0);
		_defineProperty(this, "_research", void 0);
		const opts = typeof options === "string" ? { apiKey: options } : options;
		const apiKey = ((_ref11 = (_opts$apiKey = opts.apiKey) !== null && _opts$apiKey !== void 0 ? _opts$apiKey : process.env.FIRECRAWL_API_KEY) !== null && _ref11 !== void 0 ? _ref11 : "").trim();
		const apiUrl = ((_ref12 = (_opts$apiUrl = opts.apiUrl) !== null && _opts$apiUrl !== void 0 ? _opts$apiUrl : process.env.FIRECRAWL_API_URL) !== null && _ref12 !== void 0 ? _ref12 : "https://api.firecrawl.dev").replace(/\/$/, "");
	...
	*/
	async search(query, req = {}) {
		return search(this.http, _objectSpread2({ query }, req));
	}
	/**
	...
	*/
	async startCrawl(url, req = {}) {
		return startCrawl(this.http, _objectSpread2({ url }, req));
	}
	/**
	...
	*/
	async crawl(url, req = {}) {
		return crawl(this.http, _objectSpread2({ url }, req), req.pollInterval, req.timeout);
	}
	/**
...
var FirecrawlApp = class {
	...
	async getVersion() {
		try {
			if (typeof process !== "undefined" && process.env && process.env.npm_package_version) return process.env.npm_package_version;
			return (await Promise.resolve().then(() => package_L37ZVTF4_exports)).default.version;
		} catch (error) {
	...
	async init() {
		var _this66 = this;
		_this66.version = await _this66.getVersion();
	}
	/**
	...
	*/
	constructor({ apiKey = null, apiUrl = null }) {
		_defineProperty(this, "apiKey", void 0);
		_defineProperty(this, "apiUrl", void 0);
		_defineProperty(this, "version", "1.25.1");
	...
	*/
	async scrapeUrl(url, params) {
		var _this67 = this;
		var _jsonData$extract, _jsonData$jsonOptions;
		const headers = {
			"Content-Type": "application/json",
			Authorization: `Bearer ${_this67.apiKey}`
		};
		let jsonData = _objectSpread2(_objectSpread2({ url }, params), {}, { origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this67.version}` });
		if (jsonData === null || jsonData === void 0 || (_jsonData$extract = jsonData.extract) === null || _jsonData$extract === void 0 ? void 0 : _jsonData$extract.schema) jsonData = _objectSpread2(_objectSpread2({}, jsonData), {}, { extract: _objectSpread2(_objectSpread2({}, jsonData.extract), {}, { schema: zodSchemaToJsonSchema(jsonData.extract.schema) }) });
		if (jsonData === null || jsonData === void 0 || (_jsonData$jsonOptions = jsonData.jsonOptions) === null || _jsonData$jsonOptions === void 0 ? void 0 : _jsonData$jsonOptions.schema) jsonData = _objectSpread2(_objectSpread2({}, jsonData), {}, { jsonOptions: _objectSpread2(_objectSpread2({}, jsonData.jsonOptions), {}, { schema: zodSchemaToJsonSchema(jsonData.jsonOptions.schema) }) });
		try {
			const response = await axios.post(_this67.apiUrl + `/v1/scrape`, jsonData, {
				headers,
				timeout: (params === null || params === void 0 ? void 0 : params.timeout) !== void 0 ? params.timeout + 5e3 : void 0
			});
			if (response.status === 200) {
				const responseData = response.data;
				if (responseData.success) return _objectSpread2({
					success: true,
					warning: responseData.warning,
					error: responseData.error
				}, responseData.data);
				else throw new FirecrawlError(`Failed to scrape URL. Error: ${responseData.error}`, response.status);
			} else _this67.handleError(response, "scrape URL");
		} catch (error) {
			_this67.handleError(error.response, "scrape URL");
		}
		return {
			success: false,
	...
	*/
	async search(query, params) {
		var _this68 = this;
		var _params$limit, _params$lang, _params$country, _params$timeout, _params$scrapeOptions, _jsonData$scrapeOptio;
		const headers = {
			"Content-Type": "application/json",
			Authorization: `Bearer ${_this68.apiKey}`
		};
		let jsonData = {
			query,
			limit: (_params$limit = params === null || params === void 0 ? void 0 : params.limit) !== null && _params$limit !== void 0 ? _params$limit : 5,
			tbs: params === null || params === void 0 ? void 0 : params.tbs,
			filter: params === null || params === void 0 ? void 0 : params.filter,
			lang: (_params$lang = params === null || params === void 0 ? void 0 : params.lang) !== null && _params$lang !== void 0 ? _params$lang : "en",
			country: (_params$country = params === null || params === void 0 ? void 0 : params.country) !== null && _params$country !== void 0 ? _params$country : "us",
			location: params === null || params === void 0 ? void 0 : params.location,
			origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this68.version}`,
			timeout: (_params$timeout = params === null || params === void 0 ? void 0 : params.timeout) !== null && _params$timeout !== void 0 ? _params$timeout : 6e4,
			scrapeOptions: (_params$scrapeOptions = params === null || params === void 0 ? void 0 : params.scrapeOptions) !== null && _params$scrapeOptions !== void 0 ? _params$scrapeOptions : { formats: [] }
		};
		if (jsonData === null || jsonData === void 0 || (_jsonData$scrapeOptio = jsonData.scrapeOptions) === null || _jsonData$scrapeOptio === void 0 || (_jsonData$scrapeOptio = _jsonData$scrapeOptio.extract) === null || _jsonData$scrapeOptio === void 0 ? void 0 : _jsonData$scrapeOptio.schema) jsonData = _objectSpread2(_objectSpread2({}, jsonData), {}, { scrapeOptions: _objectSpread2(_objectSpread2({}, jsonData.scrapeOptions), {}, { extract: _objectSpread2(_objectSpread2({}, jsonData.scrapeOptions.extract), {}, { schema: zodSchemaToJsonSchema(jsonData.scrapeOptions.extract.schema) }) }) });
		try {
			const response = await _this68.postRequest(_this68.apiUrl + `/v1/search`, jsonData, headers);
			if (response.status === 200) {
				const responseData = response.data;
			...
			} else _this68.handleError(response, "search");
		} catch (error) {
			var _error$response;
			if ((_error$response = error.response) === null || _error$response === void 0 || (_error$response = _error$response.data) === null || _error$response === void 0 ? void 0 : _error$response.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
	...
	*/
	async crawlUrl(url, params, pollInterval = 2, idempotencyKey) {
		var _this69 = this;
		const headers = _this69.prepareHeaders(idempotencyKey);
		let jsonData = _objectSpread2(_objectSpread2({ url }, params), {}, { origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this69.version}` });
		try {
			const response = await _this69.postRequest(_this69.apiUrl + `/v1/crawl`, jsonData, headers);
			if (response.status === 200) {
				const id = response.data.id;
				return _this69.monitorJobStatus(id, headers, pollInterval);
			} else _this69.handleError(response, "start crawl job");
		} catch (error) {
			var _error$response2;
			if ((_error$response2 = error.response) === null || _error$response2 === void 0 || (_error$response2 = _error$response2.data) === null || _error$response2 === void 0 ? void 0 : _error$response2.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	async asyncCrawlUrl(url, params, idempotencyKey) {
		var _this70 = this;
		const headers = _this70.prepareHeaders(idempotencyKey);
		let jsonData = _objectSpread2(_objectSpread2({ url }, params), {}, { origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this70.version}` });
		try {
			const response = await _this70.postRequest(_this70.apiUrl + `/v1/crawl`, jsonData, headers);
			if (response.status === 200) return response.data;
			else _this70.handleError(response, "start crawl job");
		} catch (error) {
			var _error$response3;
			if ((_error$response3 = error.response) === null || _error$response3 === void 0 || (_error$response3 = _error$response3.data) === null || _error$response3 === void 0 ? void 0 : _error$response3.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async checkCrawlStatus(id, getAllData = false, nextURL, skip, limit) {
		...
		const headers = _this71.prepareHeaders();
		const targetURL = new URL(nextURL !== null && nextURL !== void 0 ? nextURL : `${_this71.apiUrl}/v1/crawl/${id}`);
		...
		try {
			const response = await _this71.getRequest(targetURL.href, headers);
			if (response.status === 200) {
				...
				if (getAllData && response.data.status === "completed") {
					...
					if ("data" in statusData) {
						...
						while (typeof statusData === "object" && "next" in statusData) {
							...
							statusData = (await _this71.getRequest(statusData.next, headers)).data;
				...
				if (!response.data.success && response.data.error) resp = _objectSpread2(_objectSpread2({}, resp), {}, {
					success: false,
					error: response.data.error
				});
				if (response.data.next) resp.next = response.data.next;
				return resp;
			} else _this71.handleError(response, "check crawl status");
		} catch (error) {
	...
	*/
	async checkCrawlErrors(id) {
		var _this72 = this;
		const headers = _this72.prepareHeaders();
		try {
			const response = await _this72.deleteRequest(`${_this72.apiUrl}/v1/crawl/${id}/errors`, headers);
			if (response.status === 200) return response.data;
			else _this72.handleError(response, "check crawl errors");
		} catch (error) {
			throw new FirecrawlError(error.message, 500);
		}
		return {
	...
	*/
	async cancelCrawl(id) {
		var _this73 = this;
		const headers = _this73.prepareHeaders();
		try {
			const response = await _this73.deleteRequest(`${_this73.apiUrl}/v1/crawl/${id}`, headers);
			if (response.status === 200) return response.data;
			else _this73.handleError(response, "cancel crawl job");
		} catch (error) {
			throw new FirecrawlError(error.message, 500);
		}
		return {
	...
	*/
	async crawlUrlAndWatch(url, params, idempotencyKey) {
		var _this74 = this;
		const crawl2 = await _this74.asyncCrawlUrl(url, params, idempotencyKey);
		if (crawl2.success && crawl2.id) {
			const id = crawl2.id;
			return new CrawlWatcher(id, _this74);
		}
	...
	*/
	async mapUrl(url, params) {
		var _this75 = this;
		const headers = _this75.prepareHeaders();
		let jsonData = _objectSpread2(_objectSpread2({ url }, params), {}, { origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this75.version}` });
		try {
			const response = await _this75.postRequest(_this75.apiUrl + `/v1/map`, jsonData, headers);
			if (response.status === 200) return response.data;
			else _this75.handleError(response, "map");
		} catch (error) {
			throw new FirecrawlError(error.message, 500);
		}
		return {
	...
	*/
	async batchScrapeUrls(urls, params, pollInterval = 2, idempotencyKey, webhook, ignoreInvalidURLs, maxConcurrency) {
		var _this76 = this;
		var _jsonData$extract2, _jsonData$jsonOptions2;
		const headers = _this76.prepareHeaders(idempotencyKey);
		let jsonData = _objectSpread2(_objectSpread2({
			urls,
			webhook,
			ignoreInvalidURLs,
			maxConcurrency
		}, params), {}, { origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this76.version}` });
		if (jsonData === null || jsonData === void 0 || (_jsonData$extract2 = jsonData.extract) === null || _jsonData$extract2 === void 0 ? void 0 : _jsonData$extract2.schema) jsonData = _objectSpread2(_objectSpread2({}, jsonData), {}, { extract: _objectSpread2(_objectSpread2({}, jsonData.extract), {}, { schema: zodSchemaToJsonSchema(jsonData.extract.schema) }) });
		if (jsonData === null || jsonData === void 0 || (_jsonData$jsonOptions2 = jsonData.jsonOptions) === null || _jsonData$jsonOptions2 === void 0 ? void 0 : _jsonData$jsonOptions2.schema) jsonData = _objectSpread2(_objectSpread2({}, jsonData), {}, { jsonOptions: _objectSpread2(_objectSpread2({}, jsonData.jsonOptions), {}, { schema: zodSchemaToJsonSchema(jsonData.jsonOptions.schema) }) });
		try {
			const response = await _this76.postRequest(_this76.apiUrl + `/v1/batch/scrape`, jsonData, headers);
			if (response.status === 200) {
				const id = response.data.id;
				return _this76.monitorJobStatus(id, headers, pollInterval);
			} else _this76.handleError(response, "start batch scrape job");
		} catch (error) {
	...
	async asyncBatchScrapeUrls(urls, params, idempotencyKey, webhook, ignoreInvalidURLs) {
		var _this77 = this;
		const headers = _this77.prepareHeaders(idempotencyKey);
		let jsonData = _objectSpread2(_objectSpread2({
			urls,
			webhook,
			ignoreInvalidURLs
		}, params), {}, { origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this77.version}` });
		try {
			const response = await _this77.postRequest(_this77.apiUrl + `/v1/batch/scrape`, jsonData, headers);
			if (response.status === 200) return response.data;
			else _this77.handleError(response, "start batch scrape job");
		} catch (error) {
			var _error$response5;
			if ((_error$response5 = error.response) === null || _error$response5 === void 0 || (_error$response5 = _error$response5.data) === null || _error$response5 === void 0 ? void 0 : _error$response5.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async batchScrapeUrlsAndWatch(urls, params, idempotencyKey, webhook, ignoreInvalidURLs) {
		var _this78 = this;
		const crawl2 = await _this78.asyncBatchScrapeUrls(urls, params, idempotencyKey, webhook, ignoreInvalidURLs);
		if (crawl2.success && crawl2.id) {
			const id = crawl2.id;
			return new CrawlWatcher(id, _this78);
		}
	...
	*/
	async checkBatchScrapeStatus(id, getAllData = false, nextURL, skip, limit) {
		...
		const headers = _this79.prepareHeaders();
		const targetURL = new URL(nextURL !== null && nextURL !== void 0 ? nextURL : `${_this79.apiUrl}/v1/batch/scrape/${id}`);
		if (skip !== void 0) targetURL.searchParams.set("skip", skip.toString());
		if (limit !== void 0) targetURL.searchParams.set("limit", limit.toString());
		try {
			const response = await _this79.getRequest(targetURL.href, headers);
			if (response.status === 200) {
				let allData = response.data.data;
				if (getAllData && response.data.status === "completed") {
					let statusData = response.data;
					if ("data" in statusData) {
						...
						while (typeof statusData === "object" && "next" in statusData) {
							if (data.length === 0) break;
							statusData = (await _this79.getRequest(statusData.next, headers)).data;
							data = data.concat(statusData.data);
						}
						allData = data;
					}
				...
				if (response.data.next) resp.next = response.data.next;
				return resp;
			} else _this79.handleError(response, "check batch scrape status");
		} catch (error) {
			throw new FirecrawlError(error.message, 500);
		}
		return {
	...
	*/
	async checkBatchScrapeErrors(id) {
		var _this80 = this;
		const headers = _this80.prepareHeaders();
		try {
			const response = await _this80.deleteRequest(`${_this80.apiUrl}/v1/batch/scrape/${id}/errors`, headers);
			if (response.status === 200) return response.data;
			else _this80.handleError(response, "check batch scrape errors");
		} catch (error) {
			throw new FirecrawlError(error.message, 500);
		}
		return {
	...
	*/
	async extract(urls, params) {
		var _this81 = this;
		const headers = _this81.prepareHeaders();
		let jsonData = _objectSpread2({ urls }, params);
		const jsonSchema = (params === null || params === void 0 ? void 0 : params.schema) ? zodSchemaToJsonSchema(params.schema) : void 0;
		try {
			const response = await _this81.postRequest(_this81.apiUrl + `/v1/extract`, _objectSpread2(_objectSpread2({}, jsonData), {}, {
				schema: jsonSchema,
				origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this81.version}`
			}), headers);
			if (response.status === 200) {
				const jobId = response.data.id;
				let extractStatus;
				do {
					const statusResponse = await _this81.getRequest(`${_this81.apiUrl}/v1/extract/${jobId}`, headers);
					...
					if (extractStatus.status === "completed") if (extractStatus.success) return {
						...
						sources: (extractStatus === null || extractStatus === void 0 ? void 0 : extractStatus.sources) || void 0
					};
			...
			} else _this81.handleError(response, "extract");
		} catch (error) {
			var _error$response6;
			throw new FirecrawlError(error.message, 500, (_error$response6 = error.response) === null || _error$response6 === void 0 || (_error$response6 = _error$response6.data) === null || _error$response6 === void 0 ? void 0 : _error$response6.details);
		}
		return {
			success: false,
			error: "Internal server error."
	...
	*/
	async asyncExtract(urls, params, idempotencyKey) {
		var _this82 = this;
		const headers = _this82.prepareHeaders(idempotencyKey);
		const jsonData = _objectSpread2({ urls }, params);
		const jsonSchema = (params === null || params === void 0 ? void 0 : params.schema) ? zodSchemaToJsonSchema(params.schema) : void 0;
		try {
			const response = await _this82.postRequest(_this82.apiUrl + `/v1/extract`, _objectSpread2(_objectSpread2({}, jsonData), {}, {
				schema: jsonSchema,
				origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this82.version}`
			}), headers);
			if (response.status === 200) return response.data;
			else _this82.handleError(response, "start extract job");
		} catch (error) {
			var _error$response7;
			throw new FirecrawlError(error.message, 500, (_error$response7 = error.response) === null || _error$response7 === void 0 || (_error$response7 = _error$response7.data) === null || _error$response7 === void 0 ? void 0 : _error$response7.details);
		}
		return {
			success: false,
			error: "Internal server error."
	...
	*/
	async getExtractStatus(jobId) {
		var _this83 = this;
		try {
			const response = await _this83.getRequest(`${_this83.apiUrl}/v1/extract/${jobId}`, _this83.prepareHeaders());
			if (response.status === 200) return response.data;
			else _this83.handleError(response, "get extract status");
		} catch (error) {
			throw new FirecrawlError(error.message, 500);
		}
	}
	...
	*/
	postRequest(url, data, headers) {
		return axios.post(url, data, {
			headers,
			timeout: (data === null || data === void 0 ? void 0 : data.timeout) ? data.timeout + 5e3 : void 0
		});
	}
	/**
	...
	*/
	async monitorJobStatus(id, headers, checkInterval) {
		...
		while (true) try {
			let statusResponse = await _this84.getRequest(`${_this84.apiUrl}/v1/crawl/${id}`, headers);
			if (statusResponse.status === 200) {
				...
				if (statusData.status === "completed") if ("data" in statusData) {
					...
					while (typeof statusData === "object" && "next" in statusData) {
						if (data.length === 0) break;
						statusResponse = await _this84.getRequest(statusData.next, headers);
				...
				failedTries++;
				if (failedTries >= 3) _this84.handleError(statusResponse, "check crawl status");
			}
		} catch (error) {
			if (_this84.isRetryableError(error) && networkRetries < maxNetworkRetries) {
				networkRetries++;
	...
	isRetryableError(error) {
		if (error instanceof AxiosError) {
			...
			if (!error.response) {
				...
				const message = ((_error$message = error.message) === null || _error$message === void 0 ? void 0 : _error$message.toLowerCase()) || "";
			...
			if (((_error$response8 = error.response) === null || _error$response8 === void 0 ? void 0 : _error$response8.status) === 408 || ((_error$response9 = error.response) === null || _error$response9 === void 0 ? void 0 : _error$response9.status) === 504) return true;
		}
		if (error && typeof error === "object") {
			var _error$message2, _error$response10, _error$response11;
			const code = error.code;
			const message = ((_error$message2 = error.message) === null || _error$message2 === void 0 ? void 0 : _error$message2.toLowerCase()) || "";
			...
			if (((_error$response10 = error.response) === null || _error$response10 === void 0 ? void 0 : _error$response10.status) === 408 || ((_error$response11 = error.response) === null || _error$response11 === void 0 ? void 0 : _error$response11.status) === 504) return true;
		}
		return false;
	}
	/**
	...
	*/
	handleError(response, action) {
		...
		if ([
			...
			throw new FirecrawlError(`Failed to ${action}. Status code: ${response.status}. Error: ${errorMessage}${details}`, response.status, response === null || response === void 0 || (_response$data = response.data) === null || _response$data === void 0 ? void 0 : _response$data.details);
	...
	*/
	async deepResearch(query, params, onActivity, onSource) {
		var _this85 = this;
		try {
			const response = await _this85.asyncDeepResearch(query, params);
			...
			while (true) {
				researchStatus = await _this85.checkDeepResearchStatus(jobId);
			...
			throw new FirecrawlError(error.message, 500, (_error$response12 = error.response) === null || _error$response12 === void 0 || (_error$response12 = _error$response12.data) === null || _error$response12 === void 0 ? void 0 : _error$response12.details);
		}
	}
	/**
	...
	*/
	async asyncDeepResearch(query, params) {
		var _this86 = this;
		var _jsonData$jsonOptions3;
		const headers = _this86.prepareHeaders();
		let jsonData = _objectSpread2(_objectSpread2({ query }, params), {}, { origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this86.version}` });
		if (jsonData === null || jsonData === void 0 || (_jsonData$jsonOptions3 = jsonData.jsonOptions) === null || _jsonData$jsonOptions3 === void 0 ? void 0 : _jsonData$jsonOptions3.schema) jsonData = _objectSpread2(_objectSpread2({}, jsonData), {}, { jsonOptions: _objectSpread2(_objectSpread2({}, jsonData.jsonOptions), {}, { schema: zodSchemaToJsonSchema(jsonData.jsonOptions.schema) }) });
		try {
			const response = await _this86.postRequest(`${_this86.apiUrl}/v1/deep-research`, jsonData, headers);
			if (response.status === 200) return response.data;
			else _this86.handleError(response, "start deep research");
		} catch (error) {
			var _error$response13;
			if ((_error$response13 = error.response) === null || _error$response13 === void 0 || (_error$response13 = _error$response13.data) === null || _error$response13 === void 0 ? void 0 : _error$response13.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async checkDeepResearchStatus(id) {
		var _this87 = this;
		const headers = _this87.prepareHeaders();
		try {
			const response = await _this87.getRequest(`${_this87.apiUrl}/v1/deep-research/${id}`, headers);
			if (response.status === 200) return response.data;
			else if (response.status === 404) throw new FirecrawlError("Deep research job not found", 404);
			else _this87.handleError(response, "check deep research status");
		} catch (error) {
			var _error$response14;
			if ((_error$response14 = error.response) === null || _error$response14 === void 0 || (_error$response14 = _error$response14.data) === null || _error$response14 === void 0 ? void 0 : _error$response14.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async __deepResearch(topic, params, onActivity) {
		var _this88 = this;
		try {
			const response = await _this88.__asyncDeepResearch(topic, params);
			if (!response.success || "error" in response) return {
				success: false,
				error: "error" in response ? response.error : "Unknown error"
			};
			...
			let lastActivityCount = 0;
			while (true) {
				researchStatus = await _this88.__checkDeepResearchStatus(jobId);
				if ("error" in researchStatus && !researchStatus.success) return researchStatus;
				if (onActivity && researchStatus.activities) {
					const newActivities = researchStatus.activities.slice(lastActivityCount);
					for (const activity of newActivities) onActivity(activity);
			...
			throw new FirecrawlError(error.message, 500, (_error$response15 = error.response) === null || _error$response15 === void 0 || (_error$response15 = _error$response15.data) === null || _error$response15 === void 0 ? void 0 : _error$response15.details);
		}
	}
	/**
	...
	*/
	async __asyncDeepResearch(topic, params) {
		var _this89 = this;
		const headers = _this89.prepareHeaders();
		try {
			let jsonData = _objectSpread2(_objectSpread2({ topic }, params), {}, { origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this89.version}` });
			const response = await _this89.postRequest(`${_this89.apiUrl}/v1/deep-research`, jsonData, headers);
			if (response.status === 200) return response.data;
			else _this89.handleError(response, "start deep research");
		} catch (error) {
			var _error$response16;
			if ((_error$response16 = error.response) === null || _error$response16 === void 0 || (_error$response16 = _error$response16.data) === null || _error$response16 === void 0 ? void 0 : _error$response16.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async __checkDeepResearchStatus(id) {
		var _this90 = this;
		const headers = _this90.prepareHeaders();
		try {
			const response = await _this90.getRequest(`${_this90.apiUrl}/v1/deep-research/${id}`, headers);
			if (response.status === 200) return response.data;
			else if (response.status === 404) throw new FirecrawlError("Deep research job not found", 404);
			else _this90.handleError(response, "check deep research status");
		} catch (error) {
			var _error$response17;
			if ((_error$response17 = error.response) === null || _error$response17 === void 0 || (_error$response17 = _error$response17.data) === null || _error$response17 === void 0 ? void 0 : _error$response17.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async generateLLMsText(url, params) {
		var _this91 = this;
		try {
			const response = await _this91.asyncGenerateLLMsText(url, params);
			if (!response.success || "error" in response) return {
				success: false,
				error: "error" in response ? response.error : "Unknown error"
			};
			...
			while (true) {
				generationStatus = await _this91.checkGenerateLLMsTextStatus(jobId);
			...
			throw new FirecrawlError(error.message, 500, (_error$response18 = error.response) === null || _error$response18 === void 0 || (_error$response18 = _error$response18.data) === null || _error$response18 === void 0 ? void 0 : _error$response18.details);
		}
	}
	/**
	...
	*/
	async asyncGenerateLLMsText(url, params) {
		var _this92 = this;
		const headers = _this92.prepareHeaders();
		let jsonData = _objectSpread2(_objectSpread2({ url }, params), {}, { origin: typeof params.origin === "string" && params.origin.includes("mcp") ? params.origin : `js-sdk@${_this92.version}` });
		try {
			const response = await _this92.postRequest(`${_this92.apiUrl}/v1/llmstxt`, jsonData, headers);
			if (response.status === 200) return response.data;
			else _this92.handleError(response, "start LLMs.txt generation");
		} catch (error) {
			var _error$response19;
			if ((_error$response19 = error.response) === null || _error$response19 === void 0 || (_error$response19 = _error$response19.data) === null || _error$response19 === void 0 ? void 0 : _error$response19.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async checkGenerateLLMsTextStatus(id) {
		var _this93 = this;
		const headers = _this93.prepareHeaders();
		try {
			const response = await _this93.getRequest(`${_this93.apiUrl}/v1/llmstxt/${id}`, headers);
			if (response.status === 200) return response.data;
			else if (response.status === 404) throw new FirecrawlError("LLMs.txt generation job not found", 404);
			else _this93.handleError(response, "check LLMs.txt generation status");
		} catch (error) {
			var _error$response20;
			if ((_error$response20 = error.response) === null || _error$response20 === void 0 || (_error$response20 = _error$response20.data) === null || _error$response20 === void 0 ? void 0 : _error$response20.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async getQueueStatus() {
		var _this94 = this;
		const headers = _this94.prepareHeaders();
		try {
			const response = await _this94.getRequest(`${_this94.apiUrl}/v1/team/queue-status`, headers);
			if (response.status === 200) return response.data;
			else _this94.handleError(response, "get queue status");
		} catch (error) {
			var _error$response21;
			if ((_error$response21 = error.response) === null || _error$response21 === void 0 || (_error$response21 = _error$response21.data) === null || _error$response21 === void 0 ? void 0 : _error$response21.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async getCreditUsage() {
		var _this95 = this;
		const headers = _this95.prepareHeaders();
		try {
			const response = await _this95.getRequest(`${_this95.apiUrl}/v1/team/credit-usage`, headers);
			if (response.status === 200) return response.data;
			else _this95.handleError(response, "get credit usage");
		} catch (error) {
			var _error$response22;
			if ((_error$response22 = error.response) === null || _error$response22 === void 0 || (_error$response22 = _error$response22.data) === null || _error$response22 === void 0 ? void 0 : _error$response22.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async getTokenUsage() {
		var _this96 = this;
		const headers = _this96.prepareHeaders();
		try {
			const response = await _this96.getRequest(`${_this96.apiUrl}/v1/team/token-usage`, headers);
			if (response.status === 200) return response.data;
			else _this96.handleError(response, "get token usage");
		} catch (error) {
			var _error$response23;
			if ((_error$response23 = error.response) === null || _error$response23 === void 0 || (_error$response23 = _error$response23.data) === null || _error$response23 === void 0 ? void 0 : _error$response23.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async getCreditUsageHistorical(byApiKey) {
		var _this97 = this;
		const headers = _this97.prepareHeaders();
		try {
			const url = `${_this97.apiUrl}/v1/team/credit-usage/historical${byApiKey ? "?byApiKey=true" : ""}`;
			const response = await _this97.getRequest(url, headers);
			if (response.status === 200) return response.data;
			else _this97.handleError(response, "get credit usage historical");
		} catch (error) {
			var _error$response24;
			if ((_error$response24 = error.response) === null || _error$response24 === void 0 || (_error$response24 = _error$response24.data) === null || _error$response24 === void 0 ? void 0 : _error$response24.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	*/
	async getTokenUsageHistorical(byApiKey) {
		var _this98 = this;
		const headers = _this98.prepareHeaders();
		try {
			const url = `${_this98.apiUrl}/v1/team/token-usage/historical${byApiKey ? "?byApiKey=true" : ""}`;
			const response = await _this98.getRequest(url, headers);
			if (response.status === 200) return response.data;
			else _this98.handleError(response, "get token usage historical");
		} catch (error) {
			var _error$response25;
			if ((_error$response25 = error.response) === null || _error$response25 === void 0 || (_error$response25 = _error$response25.data) === null || _error$response25 === void 0 ? void 0 : _error$response25.error) throw new FirecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ""}`, error.response.status);
			else throw new FirecrawlError(error.message, 500);
		}
		return {
			success: false,
	...
	constructor(id, app) {
		...
		const messageHandler = (msg) => {
			if (msg.type === "done") {
				...
				this.data.push(...(_msg$data$data = msg.data.data) !== null && _msg$data$data !== void 0 ? _msg$data$data : []);
				for (const doc of this.data) this.dispatchTypedEvent("document", new CustomEvent("document", { detail: _objectSpread2(_objectSpread2({}, doc), {}, { id: this.id }) }));
			} else if (msg.type === "document") this.dispatchTypedEvent("document", new CustomEvent("document", { detail: _objectSpread2(_objectSpread2({}, msg.data), {}, { id: this.id }) }));
		};
...
var Firecrawl = class extends FirecrawlClient {
	...
	constructor(opts = {}) {
		...
		_defineProperty(this, "_v1Opts", void 0);
		this._v1Opts = {
...
//#endregion
//#region node_modules/@mendable/firecrawl-js/dist/package-L37ZVTF4.js
var package_L37ZVTF4_exports = /* @__PURE__ */ __exportAll({ default: () => package_L37ZVTF4_default });
var package_L37ZVTF4_default = require_package();
//#endregion
