...
function stripInlinedStylesheetAssetsFromRoute(inlineStyles, route, inlineCssHrefs, seenInlineCssHrefs) {
	...
	if (!css) return route;
	if (css.length === 0) {
		const nextRoute = _objectSpread2({}, route);
	...
	if (cssLinks.length > 0) return _objectSpread2(_objectSpread2({}, route), {}, { css: cssLinks });
	const nextRoute = _objectSpread2({}, route);
	delete nextRoute.css;
	return nextRoute;
}
...
function mergeRequestAssetsIntoRootRoute(rootRoute, requestAssets) {
	...
	return _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, rootRoute !== null && rootRoute !== void 0 ? rootRoute : {}), (preloads === null || preloads === void 0 ? void 0 : preloads.length) ? { preloads } : {}), (scripts === null || scripts === void 0 ? void 0 : scripts.length) ? { scripts } : {}), (cssLinks === null || cssLinks === void 0 ? void 0 : cssLinks.length) ? { css: cssLinks } : {});
}
function attachRouterServerSsrUtils({ router, manifest, getRequestAssets }) {
	...
	router.ssr = { get manifest() {
		...
		if (manifest.inlineCss) {
			...
			if (preparedManifest.hasStrippedRoutes) routes = _objectSpread2(_objectSpread2({}, manifest.routes), preparedManifest.routes);
		}
		if (!hasAssets) return _objectSpread2(_objectSpread2(_objectSpread2({}, manifest.scriptFormat ? { scriptFormat: manifest.scriptFormat } : {}), inlineCssAsset ? { inlineStyle: inlineCssAsset } : {}), {}, { routes });
		const rootRoute = routes[rootRouteId];
		return _objectSpread2(_objectSpread2(_objectSpread2({}, manifest.scriptFormat ? { scriptFormat: manifest.scriptFormat } : {}), inlineCssAsset ? { inlineStyle: inlineCssAsset } : {}), {}, { routes: _objectSpread2(_objectSpread2({}, routes), {}, { [rootRouteId]: mergeRequestAssetsIntoRootRoute(rootRoute, requestAssets) }) });
	} };
	...
	const serverSsr = {
		...
		dehydrate: async (opts) => {
			...
			if (manifest) {
				...
				manifestToDehydrate = _objectSpread2(_objectSpread2(_objectSpread2({}, manifest.scriptFormat ? { scriptFormat: manifest.scriptFormat } : {}), preparedManifest.inlineCssHrefs ? { inlineStyle: createInlineCssPlaceholderAsset() } : {}), {}, { routes: preparedManifest.routes });
				const requestAssets = opts === null || opts === void 0 ? void 0 : opts.requestAssets;
				if (hasRequestAssets(requestAssets)) {
					const existingRoot = manifestToDehydrate.routes[rootRouteId];
					manifestToDehydrate.routes = _objectSpread2(_objectSpread2({}, manifestToDehydrate.routes), {}, { [rootRouteId]: mergeRequestAssetsIntoRootRoute(existingRoot, requestAssets) });
				}
			}
