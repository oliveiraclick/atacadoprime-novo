...
*/
function interpolatePath(_ref) {
	let { path, params, decoder } = _ref;
...
var buildMatchContext = (inner, index, includeCurrentMatch = true) => {
	...
	const context = _objectSpread2({}, (_inner$router$options = inner.router.options.context) !== null && _inner$router$options !== void 0 ? _inner$router$options : {});
...
var handleRedirectAndNotFound = (inner, match, err) => {
	...
	if (match) {
		...
		inner.updateMatch(match.id, (prev) => _objectSpread2(_objectSpread2({}, prev), {}, {
...
var syncMatchContext = (inner, matchId, index) => {
	...
	inner.updateMatch(matchId, (prev) => {
		return _objectSpread2(_objectSpread2({}, prev), {}, { context: nextContext });
	});
};
...
var executeBeforeLoad = (inner, matchId, index, route) => {
	...
	const pending = () => {
		if (isPending) return;
		isPending = true;
		inner.updateMatch(matchId, (prev) => _objectSpread2(_objectSpread2({}, prev), {}, {
	...
	const resolve = () => {
		...
		match._nonReactive.beforeLoadPromise = void 0;
		inner.updateMatch(matchId, (prev) => _objectSpread2(_objectSpread2({}, prev), {}, { isFetching: false }));
	};
	...
	const context = _objectSpread2(_objectSpread2({}, buildMatchContext(inner, index, false)), match.__routeContext);
	const { search, params, cause } = match;
	const preload = resolvePreload(inner, matchId);
	const beforeLoadFnContext = _objectSpread2({
		search,
		abortController,
		params,
		preload,
		context,
		location: inner.location,
		navigate: (opts) => inner.router.navigate(_objectSpread2(_objectSpread2({}, opts), {}, { _fromLocation: inner.location })),
	...
	const updateContext = (beforeLoadContext) => {
		...
		inner.router.batch(() => {
			pending();
			inner.updateMatch(matchId, (prev) => _objectSpread2(_objectSpread2({}, prev), {}, { __beforeLoadContext: beforeLoadContext }));
			resolve();
		});
	};
...
var runLoader = async (inner, matchPromises, matchId, index, route) => {
	try {
		...
		try {
			...
			if (!!(loaderResultIsPromise || route._lazyPromise || route._componentsPromise || route.options.head || route.options.scripts || route.options.headers || match._nonReactive.minPendingPromise)) inner.updateMatch(matchId, (prev) => _objectSpread2(_objectSpread2({}, prev), {}, { isFetching: "loader" }));
			if (loader) {
				const loaderData = loaderResultIsPromise ? await loaderResult : loaderResult;
				handleRedirectAndNotFound(inner, inner.router.getMatch(matchId), loaderData);
				if (loaderData !== void 0) inner.updateMatch(matchId, (prev) => _objectSpread2(_objectSpread2({}, prev), {}, { loaderData }));
			}
...
var loadRouteMatch = async (inner, matchPromises, index) => {
	...
	if (shouldSkipLoader(inner, matchId)) {
		...
		if (prevMatch._nonReactive.loaderPromise) {
			...
			if (nextPreload !== match.preload) inner.updateMatch(matchId, (prev) => _objectSpread2(_objectSpread2({}, prev), {}, { preload: nextPreload }));
...
async function loadMatches(arg) {
	...
	if (notFoundToThrow) {
		...
		inner.updateMatch(boundaryMatch.id, (prev) => _objectSpread2(_objectSpread2(_objectSpread2({}, prev), boundaryIsRoot ? {
			status: "success",
			globalNotFound: true,
			error: void 0
		} : {
		...
		if (!rootMatch.globalNotFound) {
			...
			if ((_inner$router$getMatc = inner.router.getMatch(rootMatch.id)) === null || _inner$router$getMatc === void 0 ? void 0 : _inner$router$getMatc.globalNotFound) inner.updateMatch(rootMatch.id, (prev) => _objectSpread2(_objectSpread2({}, prev), {}, {
	...
	for (let i = 0; i <= headMaxIndex; i++) {
		...
		try {
			...
			if (headResult) {
				...
				inner.updateMatch(matchId, (prev) => _objectSpread2(_objectSpread2({}, prev), head));
			}
		} catch (err) {
...
function loadRouteChunk(route, componentTypesToLoad = componentTypes) {
	if (!route._lazyLoaded && route._lazyPromise === void 0) if (route.lazyFn) route._lazyPromise = route.lazyFn().then((lazyRoute) => {
		const _lazyRoute$options = lazyRoute.options, { id: _id } = _lazyRoute$options, options = _objectWithoutProperties(_lazyRoute$options, _excluded$6);
...
init_objectSpread2();
var _excluded$5 = ["from", "params"], _excluded2$3 = ["viewTransition", "ignoreBlocker"], _excluded3$2 = ["maskedLocation", "hashScrollIntoView"], _excluded4$2 = [
	"replace",
	"resetScroll",
	"hashScrollIntoView",
	"viewTransition",
...
var RouterCore = class {
	...
	*/
	constructor(options, getStoreConfig) {
		...
		this.update = (newOptions) => {
			...
			this.options = _objectSpread2(_objectSpread2({}, prevOptions), newOptions);
		...
		this.parseLocation = (locationToParse, previousLocation) => {
			...
			if (__tempLocation && (!__tempKey || __tempKey === this.tempLocationKey)) {
				...
				return _objectSpread2(_objectSpread2({}, parsedTempLocation), {}, { maskedLocation: location });
			}
			return location;
		};
		...
		this.buildLocation = (opts) => {
				...
				if (opts._includeValidateSearch && ((_this$options$search = this.options.search) === null || _this$options$search === void 0 ? void 0 : _this$options$search.strict)) {
					...
					destRoutes.forEach((route) => {
						if (route.options.validateSearch) try {
							Object.assign(validatedSearch, validateSearch(route.options.validateSearch, _objectSpread2(_objectSpread2({}, validatedSearch), nextSearch)));
						} catch (_unused3) {}
					});
				...
				if (!maskedNext) {
					...
					if (this.options.routeMasks) {
						...
						if (match) {
							...
							const _match$route = match.route, { from: _from, params: maskParams } = _match$route, maskProps = _objectWithoutProperties(_match$route, _excluded$5);
							...
							maskedDest = _objectSpread2(_objectSpread2({ from: opts.from }, maskProps), {}, { params: nextParams });
			...
			if (opts.mask) return buildWithMatches(opts, _objectSpread2({ from: opts.from }, opts.mask));
			return buildWithMatches(opts);
		};
		this.commitLocation = async (_ref2) => {
			var _next$resetScroll;
			let { viewTransition, ignoreBlocker } = _ref2, next = _objectWithoutProperties(_ref2, _excluded2$3);
			let historyAction;
			const isSameState = () => {
				const ignoredProps = [
					"key",
				...
				ignoredProps.forEach((prop) => {
					next.state[prop] = _this.latestLocation.state[prop];
				});
				const isEqual = deepEqual(next.state, _this.latestLocation.state);
				ignoredProps.forEach((prop) => {
					delete next.state[prop];
				});
				return isEqual;
			};
			const isSameUrl = trimPathRight(_this.latestLocation.href) === trimPathRight(next.href);
			let previousCommitPromise = _this.commitLocationPromise;
			_this.commitLocationPromise = createControlledPromise(() => {
				previousCommitPromise === null || previousCommitPromise === void 0 || previousCommitPromise.resolve();
				previousCommitPromise = void 0;
			});
			if (isSameUrl && isSameState()) _this.load();
			else {
				var _ref4;
				let { maskedLocation, hashScrollIntoView } = next, nextHistory = _objectWithoutProperties(next, _excluded3$2);
				if (maskedLocation) {
					var _ref3, _nextHistory$unmaskOn;
					nextHistory = _objectSpread2(_objectSpread2({}, maskedLocation), {}, { state: _objectSpread2(_objectSpread2({}, maskedLocation.state), {}, {
						__tempKey: void 0,
						__tempLocation: _objectSpread2(_objectSpread2({}, nextHistory), {}, {
							search: nextHistory.searchStr,
							state: _objectSpread2(_objectSpread2({}, nextHistory.state), {}, {
								__tempKey: void 0,
					...
					if ((_ref3 = (_nextHistory$unmaskOn = nextHistory.unmaskOnReload) !== null && _nextHistory$unmaskOn !== void 0 ? _nextHistory$unmaskOn : _this.options.unmaskOnReload) !== null && _ref3 !== void 0 ? _ref3 : false) nextHistory.state.__tempKey = _this.tempLocationKey;
				}
				nextHistory.state.__hashScrollIntoViewOptions = (_ref4 = hashScrollIntoView !== null && hashScrollIntoView !== void 0 ? hashScrollIntoView : _this.options.defaultHashScrollIntoView) !== null && _ref4 !== void 0 ? _ref4 : true;
				_this.shouldViewTransition = viewTransition;
				historyAction = next.replace ? "REPLACE" : "PUSH";
				_this.history[historyAction === "REPLACE" ? "replace" : "push"](nextHistory.publicHref, nextHistory.state, { ignoreBlocker });
			}
			_this._scroll.next = (_next$resetScroll = next.resetScroll) !== null && _next$resetScroll !== void 0 ? _next$resetScroll : true;
			if (!_this.history.subscribers.size) _this.load(historyAction ? { action: { type: historyAction } } : void 0);
			return _this.commitLocationPromise;
		};
		this.buildAndCommitLocation = (_ref5 = {}) => {
			let { replace, resetScroll, hashScrollIntoView, viewTransition, ignoreBlocker, href } = _ref5, rest = _objectWithoutProperties(_ref5, _excluded4$2);
			if (href) {
			...
			const location = this.buildLocation(_objectSpread2(_objectSpread2({}, rest), {}, { _includeValidateSearch: true }));
			this.pendingBuiltLocation = location;
			const commitPromise = this.commitLocation(_objectSpread2(_objectSpread2({}, location), {}, {
				viewTransition,
				replace,
				resetScroll,
				hashScrollIntoView,
		...
		this.navigate = async (_ref6) => {
			let { to, reloadDocument, href, publicHref } = _ref6, rest = _objectWithoutProperties(_ref6, _excluded5);
			let hrefIsUrl = false;
			if (href) try {
				new URL(`${href}`);
				hrefIsUrl = true;
			...
			if (reloadDocument) {
				if (to !== void 0 || !href) {
					...
					const location = _this.buildLocation(_objectSpread2({ to }, rest));
				...
				if (isDangerousProtocol(reloadHref, _this.protocolAllowlist)) return;
				if (!rest.ignoreBlocker) {
					var _this$history$getBloc, _this$history$getBloc2, _this$history;
					const blockers = (_this$history$getBloc = (_this$history$getBloc2 = (_this$history = _this.history).getBlockers) === null || _this$history$getBloc2 === void 0 ? void 0 : _this$history$getBloc2.call(_this$history)) !== null && _this$history$getBloc !== void 0 ? _this$history$getBloc : [];
					for (const blocker of blockers) if (blocker === null || blocker === void 0 ? void 0 : blocker.blockerFn) {
						if (await blocker.blockerFn({
							currentLocation: _this.latestLocation,
							nextLocation: _this.latestLocation,
							action: "PUSH"
						})) return;
					}
				}
			...
			return _this.buildAndCommitLocation(_objectSpread2(_objectSpread2({}, rest), {}, {
				href,
				to,
				_isNavigate: true
			}));
		...
		this.load = async (opts) => {
			...
			let notFound;
			let loadPromise;
			const previousLocation = (_this$stores$resolved = _this.stores.resolvedLocation.get()) !== null && _this$stores$resolved !== void 0 ? _this$stores$resolved : _this.stores.location.get();
			loadPromise = new Promise((resolve) => {
				_this.startTransition(async () => {
					try {
						_this.beforeLoad();
						if (historyAction) _this._scroll.hash = historyAction === "PUSH" || historyAction === "REPLACE";
						const next = _this.latestLocation;
						const locationChangeInfo = getLocationChangeInfo(next, _this.stores.resolvedLocation.get());
						if (!_this.stores.redirect.get()) _this.emit(_objectSpread2({ type: "onBeforeNavigate" }, locationChangeInfo));
						_this.emit(_objectSpread2({ type: "onBeforeLoad" }, locationChangeInfo));
						await loadMatches({
							router: _this,
							sync: opts === null || opts === void 0 ? void 0 : opts.sync,
							forceStaleReload: previousLocation.href === next.href,
							matches: _this.stores.pendingMatches.get(),
							location: next,
							updateMatch: _this.updateMatch,
							onReady: async () => {
								_this.startTransition(() => {
									_this.startViewTransition(async () => {
										let exitingMatches = null;
										let hookExitingMatches = null;
										let hookEnteringMatches = null;
										let hookStayingMatches = null;
										_this.batch(() => {
											const pendingMatches = _this.stores.pendingMatches.get();
											const mountPending = pendingMatches.length;
											const currentMatches = _this.stores.matches.get();
											exitingMatches = mountPending ? currentMatches.filter((match) => !_this.stores.pendingMatchStores.has(match.id)) : null;
											const pendingRouteIds = /* @__PURE__ */ new Set();
											for (const s of _this.stores.pendingMatchStores.values()) if (s.routeId) pendingRouteIds.add(s.routeId);
											const activeRouteIds = /* @__PURE__ */ new Set();
											for (const s of _this.stores.matchStores.values()) if (s.routeId) activeRouteIds.add(s.routeId);
											hookExitingMatches = mountPending ? currentMatches.filter((match) => !pendingRouteIds.has(match.routeId)) : null;
											hookEnteringMatches = mountPending ? pendingMatches.filter((match) => !activeRouteIds.has(match.routeId)) : null;
											hookStayingMatches = mountPending ? pendingMatches.filter((match) => activeRouteIds.has(match.routeId)) : currentMatches;
											_this.stores.isLoading.set(false);
											_this.stores.loadedAt.set(Date.now());
											/**
											...
											*/
											if (mountPending) {
												_this.stores.setMatches(pendingMatches);
												_this.stores.setPending([]);
												_this.stores.setCached([..._this.stores.cachedMatches.get(), ...exitingMatches.filter((d) => d.status !== "error" && d.status !== "notFound" && d.status !== "redirected")]);
												_this.clearExpiredCache();
											}
										});
										for (const [matches, hook] of [
											...
											if (!matches) continue;
											for (const match of matches) (_this$looseRoutesById = (_this$looseRoutesById2 = _this.looseRoutesById[match.routeId].options)[hook]) === null || _this$looseRoutesById === void 0 || _this$looseRoutesById.call(_this$looseRoutesById2, match);
										}
									});
								});
							}
						...
						const nextStatusCode = redirect ? redirect.status : notFound ? 404 : _this.stores.matches.get().some((d) => d.status === "error") ? 500 : 200;
						_this.batch(() => {
							_this.stores.statusCode.set(nextStatusCode);
							_this.stores.redirect.set(redirect);
						});
					}
					if (_this.latestLoadPromise === loadPromise) {
						var _this$commitLocationP;
						(_this$commitLocationP = _this.commitLocationPromise) === null || _this$commitLocationP === void 0 || _this$commitLocationP.resolve();
						_this.latestLoadPromise = void 0;
						_this.commitLocationPromise = void 0;
					}
					resolve();
				});
			});
			_this.latestLoadPromise = loadPromise;
			await loadPromise;
			while (_this.latestLoadPromise && loadPromise !== _this.latestLoadPromise) await _this.latestLoadPromise;
			let newStatusCode = void 0;
			if (_this.hasNotFoundMatch()) newStatusCode = 404;
			else if (_this.stores.matches.get().some((d) => d.status === "error")) newStatusCode = 500;
			if (newStatusCode !== void 0) _this.stores.statusCode.set(newStatusCode);
		};
		...
		this.getMatch = (matchId) => {
			var _ref7, _this$stores$cachedMa, _this$stores$cachedMa2, _this$stores$pendingM, _this$stores$matchSto2;
			return (_ref7 = (_this$stores$cachedMa = (_this$stores$cachedMa2 = this.stores.cachedMatchStores.get(matchId)) === null || _this$stores$cachedMa2 === void 0 ? void 0 : _this$stores$cachedMa2.get()) !== null && _this$stores$cachedMa !== void 0 ? _this$stores$cachedMa : (_this$stores$pendingM = this.stores.pendingMatchStores.get(matchId)) === null || _this$stores$pendingM === void 0 ? void 0 : _this$stores$pendingM.get()) !== null && _ref7 !== void 0 ? _ref7 : (_this$stores$matchSto2 = this.stores.matchStores.get(matchId)) === null || _this$stores$matchSto2 === void 0 ? void 0 : _this$stores$matchSto2.get();
		};
		this.invalidate = (opts) => {
			const invalidate = (d) => {
				var _opts$filter, _opts$filter2;
				if ((_opts$filter = opts === null || opts === void 0 || (_opts$filter2 = opts.filter) === null || _opts$filter2 === void 0 ? void 0 : _opts$filter2.call(opts, d)) !== null && _opts$filter !== void 0 ? _opts$filter : true) return _objectSpread2(_objectSpread2({}, d), {}, { invalid: true }, (opts === null || opts === void 0 ? void 0 : opts.forcePending) || d.status === "error" || d.status === "notFound" ? {
					status: "pending",
					error: void 0
				} : void 0);
				return d;
		...
		this.clearExpiredCache = () => {
			const now = Date.now();
			const filter = (d) => {
				var _ref8, _route$options$preloa, _route$options$gcTime;
				const route = this.looseRoutesById[d.routeId];
				if (!route.options.loader) return true;
				const gcTime = (_ref8 = d.preload ? (_route$options$preloa = route.options.preloadGcTime) !== null && _route$options$preloa !== void 0 ? _route$options$preloa : this.options.defaultPreloadGcTime : (_route$options$gcTime = route.options.gcTime) !== null && _route$options$gcTime !== void 0 ? _route$options$gcTime : this.options.defaultGcTime) !== null && _ref8 !== void 0 ? _ref8 : 300 * 1e3;
		...
		this.preloadRoute = async (opts) => {
			...
			const next = (_opts$_builtLocation = opts._builtLocation) !== null && _opts$_builtLocation !== void 0 ? _opts$_builtLocation : _this.buildLocation(opts);
			let matches = _this.matchRoutes(next, {
				throwOnError: true,
				preload: true,
				dest: opts
			});
			const activeMatchIds = new Set([..._this.stores.matchesId.get(), ..._this.stores.pendingIds.get()]);
			const loadedMatchIds = new Set([...activeMatchIds, ..._this.stores.cachedIds.get()]);
			const matchesToCache = matches.filter((match) => !loadedMatchIds.has(match.id));
			if (matchesToCache.length) {
				const cachedMatches = _this.stores.cachedMatches.get();
				_this.stores.setCached([...cachedMatches, ...matchesToCache]);
			}
			try {
				matches = await loadMatches({
					...
					updateMatch: (id, updater) => {
						...
						else _this.updateMatch(id, updater);
					}
				});
				return matches;
			} catch (err) {
				if (isRedirect(err)) {
					...
					return await _this.preloadRoute(_objectSpread2(_objectSpread2({}, err.options), {}, { _fromLocation: next }));
				}
		...
		this.matchRoute = (location, opts) => {
			...
			const matchLocation = _objectSpread2(_objectSpread2({}, location), {}, {
		...
		this.update(_objectSpread2(_objectSpread2({
	...
	getParentContext(parentMatch) {
		var _this$options$context, _ref9, _parentMatch$context;
		return !(parentMatch === null || parentMatch === void 0 ? void 0 : parentMatch.id) ? (_this$options$context = this.options.context) !== null && _this$options$context !== void 0 ? _this$options$context : void 0 : (_ref9 = (_parentMatch$context = parentMatch.context) !== null && _parentMatch$context !== void 0 ? _parentMatch$context : this.options.context) !== null && _ref9 !== void 0 ? _ref9 : void 0;
	}
	matchRoutesInternal(next, opts) {
		...
		for (let index = 0; index < matchedRoutes.length; index++) {
			...
			let searchError;
			{
				...
				try {
					var _validateSearch;
					const strictSearch = (_validateSearch = validateSearch(route.options.validateSearch, _objectSpread2({}, parentSearch))) !== null && _validateSearch !== void 0 ? _validateSearch : void 0;
					preMatchSearch = _objectSpread2(_objectSpread2({}, parentSearch), strictSearch);
					strictMatchSearch = _objectSpread2(_objectSpread2({}, parentStrictSearch), strictSearch);
					searchError = void 0;
				} catch (err) {
			...
			let match;
			if (existingMatch) match = _objectSpread2(_objectSpread2({}, existingMatch), {}, {
				cause,
			...
			match.context = _objectSpread2(_objectSpread2(_objectSpread2({}, parentContext), match.__routeContext), match.__beforeLoadContext);
			matches[index] = match;
		}
			...
			if (!existingMatch) {
				...
				if (route.options.context) {
					...
					const contextFnContext = {
						...
						location: next,
						navigate: (opts) => this.navigate(_objectSpread2(_objectSpread2({}, opts), {}, { _fromLocation: next })),
	...
	*/
	matchRoutesLightweight(location) {
		...
		const accumulatedSearch = _objectSpread2({}, location.search);
		...
		if (routeValidateSearch) {
			const validate = ({ search, next, meta }) => {
				...
				if (includeValidateSearch) try {
					...
					return _objectSpread2(_objectSpread2({}, result), validated);
				} catch (_unused8) {}
				return result;
			};
...
function getScriptPreloadAttrs(manifest, link, assetCrossOrigin) {
	...
	return _objectSpread2(_objectSpread2({}, getManifestScriptFormat(manifest) === "iife" ? {
		rel: "preload",
		as: "script"
	} : { rel: "modulepreload" }), {}, { href: preloadLink.href }, crossOrigin ? { crossOrigin } : {});
}
...
var BaseRoute = class {
	...
	constructor(options) {
		...
		this.redirect = (opts) => redirect(_objectSpread2({ from: this.fullPath }, opts));
...
function useLoaderDeps(opts) {
	const { select } = opts;
	return useMatch(_objectSpread2(_objectSpread2({}, _objectWithoutProperties(opts, _excluded$4)), {}, { select: (match) => {
...
function useNavigate(_defaultOpts) {
	...
	return import_react.useCallback((options) => {
		var _options$from;
		return router.navigate(_objectSpread2(_objectSpread2({}, options), {}, { from: (_options$from = options.from) !== null && _options$from !== void 0 ? _options$from : _defaultOpts === null || _defaultOpts === void 0 ? void 0 : _defaultOpts.from }));
...
function useRouteContext(opts) {
	return useMatch(_objectSpread2(_objectSpread2({}, opts), {}, { select: (match) => opts.select ? opts.select(match.context) : match.context }));
}
//#endregion
...
var _excluded$3 = [
	...
	"from",
	"_fromLocation"
], _excluded2$2 = ["_asChild"], _excluded3$1 = ["type"], _excluded4$1 = ["disabled"];
/**
...
function useLinkProps(options, forwardedRef) {
	...
	const { activeProps, inactiveProps, activeOptions, to, preload: userPreload, preloadDelay: userPreloadDelay, preloadIntentProximity: _preloadIntentProximity, hashScrollIntoView, replace, startTransition, resetScroll, viewTransition, children, target, disabled, style, className, onClick, onBlur, onFocus, onMouseEnter, onMouseLeave, onTouchStart, ignoreBlocker, params: _params, search: _search, hash: _hash, state: _state, mask: _mask, reloadDocument: _reloadDocument, unsafeRelative: _unsafeRelative, from: _from, _fromLocation } = options, propsSafeToSpread = _objectWithoutProperties(options, _excluded$3);
	{
		var _functionalUpdate, _functionalUpdate2;
		const safeInternal = isSafeInternal(to);
		if (typeof to === "string" && !safeInternal && to.indexOf(":") > -1) try {
			new URL(to);
			if (isDangerousProtocol(to, router.protocolAllowlist)) return _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, propsSafeToSpread), {}, {
				ref: innerRef,
				href: void 0
			}, children && { children }), target && { target }), disabled && { disabled }), style && { style }), className && { className });
			return _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, propsSafeToSpread), {}, {
				ref: innerRef,
				href: to
			}, children && { children }), target && { target }), disabled && { disabled }), style && { style }), className && { className });
		} catch (_unused) {}
		const next = router.buildLocation(_objectSpread2(_objectSpread2({}, options), {}, { from: options.from }));
		...
		if (externalLink) return _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, propsSafeToSpread), {}, {
			ref: innerRef,
			href: externalLink
		}, children && { children }), target && { target }), disabled && { disabled }), style && { style }), className && { className });
		...
		const resolvedStyle = (() => {
			...
			return _objectSpread2(_objectSpread2(_objectSpread2({}, baseStyle), activeStyle), inactiveStyle);
		})();
		...
		return _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, propsSafeToSpread), resolvedActiveProps), resolvedInactiveProps), {}, {
			href: hrefOption === null || hrefOption === void 0 ? void 0 : hrefOption.href,
			ref: innerRef,
			disabled: !!disabled,
			target
		}, resolvedStyle && { style: resolvedStyle }), resolvedClassName && { className: resolvedClassName }), disabled && STATIC_DISABLED_PROPS), isActive && STATIC_ACTIVE_PROPS);
	}
	...
	const next = import_react.useMemo(() => {
		const opts = _objectSpread2({ _fromLocation: currentLocation }, _options);
	...
	const resolvedStyle = (style || resolvedActiveProps.style || resolvedInactiveProps.style) && _objectSpread2(_objectSpread2(_objectSpread2({}, style), resolvedActiveProps.style), resolvedInactiveProps.style);
	...
	const doPreload = import_react.useCallback(() => {
		router.preloadRoute(_objectSpread2(_objectSpread2({}, _options), {}, { _builtLocation: next })).catch((err) => {
			console.warn(err);
	...
	const handleClick = (e) => {
		...
		if (!disabled && !isCtrlEvent(e) && !e.defaultPrevented && (!effectiveTarget || effectiveTarget === "_self") && e.button === 0) {
			...
			router.navigate(_objectSpread2(_objectSpread2({}, _options), {}, {
				replace,
				resetScroll,
				hashScrollIntoView,
				startTransition,
	...
	if (externalLink) return _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, propsSafeToSpread), {}, {
		ref: innerRef,
		href: externalLink
	}, children && { children }), target && { target }), disabled && { disabled }), style && { style }), className && { className }), onClick && { onClick }), onBlur && { onBlur }), onFocus && { onFocus }), onMouseEnter && { onMouseEnter }), onMouseLeave && { onMouseLeave }), onTouchStart && { onTouchStart });
	...
	return _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, propsSafeToSpread), resolvedActiveProps), resolvedInactiveProps), {}, {
		href: hrefOption === null || hrefOption === void 0 ? void 0 : hrefOption.href,
		ref: innerRef,
		...
		disabled: !!disabled,
		target
	}, resolvedStyle && { style: resolvedStyle }), resolvedClassName && { className: resolvedClassName }), disabled && STATIC_DISABLED_PROPS), isActive && STATIC_ACTIVE_PROPS), isHydrated && isTransitioning && STATIC_TRANSITIONING_PROPS);
}
...
var Link = import_react.forwardRef((props, ref) => {
	const { _asChild } = props, rest = _objectWithoutProperties(props, _excluded2$2);
	const _useLinkProps = useLinkProps(rest, ref), { type: _type } = _useLinkProps, linkProps = _objectWithoutProperties(_useLinkProps, _excluded3$1);
	const children = typeof rest.children === "function" ? rest.children({ isActive: linkProps["data-status"] === "active" }) : rest.children;
	if (!_asChild) {
		const { disabled: _ } = linkProps, rest = _objectWithoutProperties(linkProps, _excluded4$1);
	...
	*/
	constructor(options) {
		...
		this.useRouteContext = (opts) => {
			return useRouteContext(_objectSpread2(_objectSpread2({}, opts), {}, { from: this.id }));
		};
		this.useSearch = (opts) => {
			return useSearch({
				select: opts === null || opts === void 0 ? void 0 : opts.select,
		...
		this.useLoaderDeps = (opts) => {
			return useLoaderDeps(_objectSpread2(_objectSpread2({}, opts), {}, { from: this.id }));
		};
		this.useLoaderData = (opts) => {
			return useLoaderData(_objectSpread2(_objectSpread2({}, opts), {}, { from: this.id }));
		};
		this.useNavigate = () => {
			return useNavigate({ from: this.fullPath });
		};
		this.Link = import_react.forwardRef((props, ref) => {
			return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Link, _objectSpread2({
				ref,
				from: this.fullPath
			}, props));
		});
...
function renderRouteNotFound(router, route, data) {
	if (!route.options.notFoundComponent) {
		if (router.options.defaultNotFoundComponent) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(router.options.defaultNotFoundComponent, _objectSpread2({}, data));
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DefaultGlobalNotFound, {});
	}
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(route.options.notFoundComponent, _objectSpread2({}, data));
}
//#endregion
...
init_objectSpread2();
var _excluded$2 = ["router", "children"], _excluded2$1 = ["router"];
/**
* Low-level provider that places the router into React context and optionally
* updates router options from props. Most apps should use `RouterProvider`.
*/
function RouterContextProvider(_ref) {
	let { router, children } = _ref, rest = _objectWithoutProperties(_ref, _excluded$2);
	if (hasKeys(rest)) router.update(_objectSpread2(_objectSpread2(_objectSpread2({}, router.options), rest), {}, { context: _objectSpread2(_objectSpread2({}, router.options.context), rest.context) }));
...
function RouterProvider(_ref2) {
	let { router } = _ref2, rest = _objectWithoutProperties(_ref2, _excluded2$1);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RouterContextProvider, _objectSpread2(_objectSpread2({ router }, rest), {}, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Matches, {}) }));
}
//#endregion
...
function Asset(asset) {
	...
	switch (asset.tag) {
		case "title": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("title", _objectSpread2(_objectSpread2({}, attrs), {}, {
			suppressHydrationWarning: true,
			children
		}));
		case "meta": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("meta", _objectSpread2(_objectSpread2({}, attrs), {}, { suppressHydrationWarning: true }));
		case "link":
			var _attrs$precedence;
			return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("link", _objectSpread2(_objectSpread2({}, attrs), {}, {
		...
		case "style":
			...
			return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("style", _objectSpread2(_objectSpread2({}, attrs), {}, {
...
function Script({ attrs, children, preventScriptHoist }) {
	...
	if (attrs === null || attrs === void 0 ? void 0 : attrs.src) {
		if (!preventScriptHoist) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("script", _objectSpread2(_objectSpread2({}, attrs), {}, { suppressHydrationWarning: true }));
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("script", _objectSpread2(_objectSpread2({}, attrs), {}, {
			onLoad: noopScriptHandler,
			suppressHydrationWarning: true
		}));
	}
	if (typeof children === "string") return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("script", _objectSpread2(_objectSpread2({}, attrs), {}, {
		dangerouslySetInnerHTML: { __html: children },
		suppressHydrationWarning: true
	}));
	return null;
...
init_objectSpread2();
var _excluded$1 = ["children"], _excluded2 = ["children"];
function buildTagsFromMatches(router, nonce, matches, assetCrossOrigin) {
	...
	for (let i = routeMeta.length - 1; i >= 0; i--) {
		...
		for (let j = metas.length - 1; j >= 0; j--) {
			...
			if (m.title) {
				...
				resultMeta.push({
					tag: "meta",
					attrs: _objectSpread2(_objectSpread2({}, m), {}, { nonce })
				});
			}
		}
	}
	...
	const constructedLinks = matches.flatMap((match) => {
		...
		tag: "link",
		attrs: _objectSpread2(_objectSpread2({}, link), {}, { nonce })
	}));
	...
	if (manifest) {
		matches.forEach((match) => {
			...
			(_manifest$routes$matc = manifest.routes[match.routeId]) === null || _manifest$routes$matc === void 0 || (_manifest$routes$matc = _manifest$routes$matc.css) === null || _manifest$routes$matc === void 0 || _manifest$routes$matc.forEach((link) => {
				...
				manifestCssTags.push({
					tag: "link",
					attrs: _objectSpread2(_objectSpread2({ rel: "stylesheet" }, resolvedLink), {}, {
		...
		if (manifest.inlineStyle) manifestCssTags.push({
			tag: "style",
			attrs: _objectSpread2(_objectSpread2({}, manifest.inlineStyle.attrs), {}, { nonce }),
	...
	if (manifest) matches.forEach((match) => {
		...
		(_manifest$routes$matc2 = manifest.routes[match.routeId]) === null || _manifest$routes$matc2 === void 0 || (_manifest$routes$matc2 = _manifest$routes$matc2.preloads) === null || _manifest$routes$matc2 === void 0 || _manifest$routes$matc2.forEach((preload) => {
			preloadLinks.push({
				tag: "link",
				attrs: _objectSpread2(_objectSpread2({}, getScriptPreloadAttrs(manifest, preload, assetCrossOrigin)), {}, { nonce })
			});
		});
	});
	...
	}).filter((style) => style !== void 0).map((_ref) => {
		let { children } = _ref;
		return {
			tag: "style",
			attrs: _objectSpread2(_objectSpread2({}, _objectWithoutProperties(_ref, _excluded$1)), {}, { nonce }),
			children
		};
	});
	...
	}).filter((script) => script !== void 0).map((_ref2) => {
		let { children } = _ref2;
		return {
			tag: "script",
			attrs: _objectSpread2(_objectSpread2({}, _objectWithoutProperties(_ref2, _excluded2)), {}, { nonce }),
			children
		};
	});
	const tags = [];
...
function HeadContent(props) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: tags.map((tag) => /* @__PURE__ */ (0, import_react.createElement)(Asset, _objectSpread2(_objectSpread2({}, tag), {}, {
...
var Scripts = () => {
	...
	const getAssetScripts = (matches) => {
		...
		for (const match of matches) {
			...
			if (!scripts) continue;
			for (const asset of scripts) assetScripts.push(_objectSpread2({
				tag: "script",
				attrs: _objectSpread2(_objectSpread2({}, asset.attrs), {}, { nonce }),
				children: asset.children
			}, typeof ((_asset$attrs = asset.attrs) === null || _asset$attrs === void 0 ? void 0 : _asset$attrs.src) === "string" ? { preventScriptHoist: true } : {}));
		}
		return assetScripts;
	};
	const getScripts = (matches) => matches.map((match) => match.scripts).flat(1).filter(Boolean).map((_ref) => {
		let { children } = _ref;
		return {
			tag: "script",
			attrs: _objectSpread2(_objectSpread2({}, _objectWithoutProperties(_ref, _excluded)), {}, {
				suppressHydrationWarning: true,
				nonce
			}),
			children
...
function renderScripts(router, scripts, assetScripts) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: allScripts.map((asset, i) => /* @__PURE__ */ (0, import_react.createElement)(Asset, _objectSpread2(_objectSpread2({}, asset), {}, { key: `tsr-scripts-${asset.tag}-${i}` }))) });
}
//#endregion
...
var renderRouterToStream = async ({ request, router, responseHeaders, children }) => {
	...
	if (typeof import_server_node.renderToPipeableStream === "function") {
		...
		try {
			...
			pipeable = import_server_node.renderToPipeableStream(children, _objectSpread2(_objectSpread2({
