...
function createCollection(name) {
	...
	const CollectionItemSlot = import_react.forwardRef((props, forwardedRef) => {
		const { scope, children } = props, itemData = _objectWithoutProperties(props, _excluded);
		const ref = import_react.useRef(null);
		const composedRefs = useComposedRefs(forwardedRef, ref);
		const context = useCollectionContext(ITEM_SLOT_NAME, scope);
		import_react.useEffect(() => {
			context.itemMap.set(ref, _objectSpread2({ ref }, itemData));
			return () => void context.itemMap.delete(ref);
		});
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CollectionItemSlotImpl, _objectSpread2(_objectSpread2({}, { [ITEM_DATA_ATTR]: "" }), {}, {
			ref: composedRefs,
			children
		}));
	});
