...
var _excluded2 = [
	...
	"flipAlignment"
], _excluded3 = ["strategy"], _excluded4 = [
	"mainAxis",
	"crossAxis",
	"limiter"
], _excluded5 = ["apply"];
...
var computePosition = async (reference, floating, config) => {
	...
	const platformWithDetectOverflow = platform.detectOverflow ? platform : _objectSpread2(_objectSpread2({}, platform), {}, { detectOverflow });
...
var hide = function(options) {
	...
	return {
		...
		async fn(state) {
			...
			const _evaluate3 = evaluate(options, state), { strategy = "referenceHidden" } = _evaluate3, detectOverflowOptions = _objectWithoutProperties(_evaluate3, _excluded3);
			switch (strategy) {
				case "referenceHidden": {
					const offsets = getSideOffsets(await platform.detectOverflow(state, _objectSpread2(_objectSpread2({}, detectOverflowOptions), {}, { elementContext: "reference" })), rects.reference);
					return { data: {
				...
				case "escaped": {
					const offsets = getSideOffsets(await platform.detectOverflow(state, _objectSpread2(_objectSpread2({}, detectOverflowOptions), {}, { altBoundary: true })), rects.floating);
					return { data: {
...
var size = function(options) {
	...
	return {
		...
		async fn(state) {
			...
			const _evaluate5 = evaluate(options, state), { apply = () => {} } = _evaluate5, detectOverflowOptions = _objectWithoutProperties(_evaluate5, _excluded5);
			const overflow = await platform.detectOverflow(state, detectOverflowOptions);
			const side = getSide(placement);
			const alignment = getAlignment(placement);
			...
			await apply(_objectSpread2(_objectSpread2({}, state), {}, {
				availableWidth,
				availableHeight
			}));
