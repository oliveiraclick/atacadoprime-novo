...
var GoTrueAdminApi = class {
	...
	*/
	async signOut(jwt, scope = SIGN_OUT_SCOPES[0]) {
		...
		try {
			await _request(_this.fetch, "POST", `${_this.url}/logout?scope=${scope}`, {
	...
	*/
	async inviteUserByEmail(email, options = {}) {
		var _this2 = this;
		try {
			return await _request(_this2.fetch, "POST", `${_this2.url}/invite`, {
				body: {
					email,
					data: options.data
				},
	...
	*/
	async generateLink(params) {
		...
		try {
			...
			return await _request(_this3.fetch, "POST", `${_this3.url}/admin/generate_link`, {
				body,
	...
	*/
	async createUser(attributes) {
		var _this4 = this;
		try {
			return await _request(_this4.fetch, "POST", `${_this4.url}/admin/users`, {
				body: attributes,
	...
	*/
	async listUsers(params) {
		...
		try {
			...
			const response = await _request(_this5.fetch, "GET", `${_this5.url}/admin/users`, {
	...
	*/
	async getUserById(uid) {
		...
		validateUUID(uid);
		try {
			return await _request(_this6.fetch, "GET", `${_this6.url}/admin/users/${uid}`, {
	...
	*/
	async updateUserById(uid, attributes) {
		...
		validateUUID(uid);
		try {
			return await _request(_this7.fetch, "PUT", `${_this7.url}/admin/users/${uid}`, {
				body: attributes,
	...
	*/
	async deleteUser(id, shouldSoftDelete = false) {
		...
		validateUUID(id);
		try {
			return await _request(_this8.fetch, "DELETE", `${_this8.url}/admin/users/${id}`, {
	...
	async _listFactors(params) {
		...
		try {
			const { data, error } = await _request(_this9.fetch, "GET", `${_this9.url}/admin/users/${params.userId}/factors`, {
	...
	async _deleteFactor(params) {
		...
		try {
			return {
				data: await _request(_this10.fetch, "DELETE", `${_this10.url}/admin/users/${params.userId}/factors/${params.id}`, { headers: _this10.headers }),
				error: null
			};
		} catch (error) {
			if (isAuthError(error)) return {
	...
	*/
	async _listOAuthClients(params) {
		...
		try {
			...
			const response = await _request(_this11.fetch, "GET", `${_this11.url}/admin/oauth/clients`, {
	...
	*/
	async _createOAuthClient(params) {
		var _this12 = this;
		try {
			return await _request(_this12.fetch, "POST", `${_this12.url}/admin/oauth/clients`, {
				body: params,
	...
	*/
	async _getOAuthClient(clientId) {
		var _this13 = this;
		try {
			return await _request(_this13.fetch, "GET", `${_this13.url}/admin/oauth/clients/${clientId}`, {
	...
	*/
	async _updateOAuthClient(clientId, params) {
		var _this14 = this;
		try {
			return await _request(_this14.fetch, "PUT", `${_this14.url}/admin/oauth/clients/${clientId}`, {
				body: params,
	...
	*/
	async _deleteOAuthClient(clientId) {
		var _this15 = this;
		try {
			await _request(_this15.fetch, "DELETE", `${_this15.url}/admin/oauth/clients/${clientId}`, {
	...
	*/
	async _regenerateOAuthClientSecret(clientId) {
		var _this16 = this;
		try {
			return await _request(_this16.fetch, "POST", `${_this16.url}/admin/oauth/clients/${clientId}/regenerate_secret`, {
	...
	*/
	async _listCustomProviders(params) {
		...
		try {
			...
			return await _request(_this17.fetch, "GET", `${_this17.url}/admin/custom-providers`, {
	...
	*/
	async _createCustomProvider(params) {
		var _this18 = this;
		try {
			return await _request(_this18.fetch, "POST", `${_this18.url}/admin/custom-providers`, {
				body: params,
	...
	*/
	async _getCustomProvider(identifier) {
		var _this19 = this;
		try {
			return await _request(_this19.fetch, "GET", `${_this19.url}/admin/custom-providers/${identifier}`, {
	...
	*/
	async _updateCustomProvider(identifier, params) {
		var _this20 = this;
		try {
			return await _request(_this20.fetch, "PUT", `${_this20.url}/admin/custom-providers/${identifier}`, {
				body: params,
	...
	*/
	async _deleteCustomProvider(identifier) {
		var _this21 = this;
		try {
			await _request(_this21.fetch, "DELETE", `${_this21.url}/admin/custom-providers/${identifier}`, {
	...
	*/
	async _adminListPasskeys(params) {
		...
		validateUUID(params.userId);
		try {
			return await _request(_this22.fetch, "GET", `${_this22.url}/admin/users/${params.userId}/passkeys`, {
	...
	*/
	async _adminDeletePasskey(params) {
		...
		try {
			await _request(_this23.fetch, "DELETE", `${_this23.url}/admin/users/${params.userId}/passkeys/${params.passkeyId}`, {
...
var WebAuthnApi = class {
	...
	*/
	async _challenge({ factorId, webauthn, friendlyName, signal }, overrides) {
		...
		var _a;
		try {
			const { data: challengeResponse, error: challengeError } = await _this2.client.mfa.challenge({
				factorId,
				webauthn
			});
			...
			if (challengeResponse.webauthn.type === "create") {
				...
				if (!user.name) {
					...
					if (!nameToUse) {
						const userData = (await _this2.client.getUser()).data.user;
	...
	*/
	async _authenticate({ factorId, webauthn: { rpId = typeof window !== "undefined" ? window.location.hostname : void 0, rpOrigins = typeof window !== "undefined" ? [window.location.origin] : void 0, signal } = {} }, overrides) {
		...
		try {
			...
			const { data: challengeResponse, error: challengeError } = await _this4.challenge({
				factorId,
				webauthn: {
					rpId,
					rpOrigins
	...
	*/
	async _register({ friendlyName, webauthn: { rpId = typeof window !== "undefined" ? window.location.hostname : void 0, rpOrigins = typeof window !== "undefined" ? [window.location.origin] : void 0, signal } = {} }, overrides) {
		...
		try {
			...
			const { data: factor, error: enrollError } = await _this5._enroll({ friendlyName });
			if (!factor) {
				await _this5.client.mfa.listFactors().then((factors) => {
					var _a;
			...
			const { data: challengeResponse, error: challengeError } = await _this5._challenge({
				factorId: factor.id,
...
var GoTrueClient = class GoTrueClient {
		...
		*/
		...
		if (isBrowser() && globalThis.BroadcastChannel && this.persistSession && this.storageKey) {
			...
			(_c = this.broadcastChannel) === null || _c === void 0 || _c.addEventListener("message", async (event) => {
				...
				try {
					await _this._notifyAllSubscribers(event.data.event, event.data.session, false);
				} catch (error) {
	...
	*/
	async initialize() {
		var _this2 = this;
		if (_this2.initializePromise) return await _this2.initializePromise;
		_this2.initializePromise = (async () => {
			if (_this2.lock != null) return await _this2._acquireLock(_this2.lockAcquireTimeout, async () => {
				return await _this2._initialize();
			});
			return await _this2._initialize();
		})();
		return await _this2.initializePromise;
	}
	/**
	* IMPORTANT:
	...
	async _initialize() {
		...
		try {
			...
			if (isBrowser()) {
				...
				else if (await _this3._isPKCECallback(params)) callbackUrlType = "pkce";
			}
			/**
			...
			*/
			if (isBrowser() && _this3.detectSessionInUrl && callbackUrlType !== "none") {
				const { data, error } = await _this3._getSessionFromURL(params, callbackUrlType);
				if (error) {
				...
				await _this3._saveSession(session);
				setTimeout(async () => {
					if (redirectType === "recovery") await _this3._notifyAllSubscribers("PASSWORD_RECOVERY", session);
					else await _this3._notifyAllSubscribers("SIGNED_IN", session);
				}, 0);
				return { error: null };
			}
			await _this3._recoverAndRefresh();
			return { error: null };
		} catch (error) {
			...
			await _this3._handleVisibilityChange();
	...
	*/
	async signInAnonymously(credentials) {
		...
		var _a, _b, _c;
		try {
			const { data, error } = await _request(_this4.fetch, "POST", `${_this4.url}/signup`, {
				headers: _this4.headers,
				body: {
			...
			if (data.session) {
				await _this4._saveSession(data.session);
				await _this4._notifyAllSubscribers("SIGNED_IN", session);
			}
	...
	async signUp(credentials) {
		...
		try {
			...
			if ("email" in credentials) {
				...
				if (_this5.flowType === "pkce") [codeChallenge, codeChallengeMethod] = await getCodeChallengeAndMethod(_this5.storage, _this5.storageKey);
				res = await _request(_this5.fetch, "POST", `${_this5.url}/signup`, {
					headers: _this5.headers,
			...
			if (error || !data) {
				await removeItemAsync(_this5.storage, `${_this5.storageKey}-code-verifier`);
			...
			const user = data.user;
			if (data.session) {
				await _this5._saveSession(data.session);
				await _this5._notifyAllSubscribers("SIGNED_IN", session);
			}
			return _this5._returnResult({
				data: {
					user,
	...
	*/
	async signInWithPassword(credentials) {
		...
		try {
			...
			if ("email" in credentials) {
				const { email, password, options } = credentials;
				res = await _request(_this6.fetch, "POST", `${_this6.url}/token?grant_type=password`, {
					headers: _this6.headers,
					body: {
						email,
						password,
			...
			if (data.session) {
				await _this6._saveSession(data.session);
				await _this6._notifyAllSubscribers("SIGNED_IN", data.session);
			}
			return _this6._returnResult({
				data: Object.assign({
					user: data.user,
	...
	*/
	async signInWithOAuth(credentials) {
		var _this7 = this;
		var _a, _b, _c, _d;
		return await _this7._handleProviderSignIn(credentials.provider, {
	...
	*/
	async exchangeCodeForSession(authCode) {
		var _this8 = this;
		await _this8.initializePromise;
		if (_this8.lock != null) return _this8._acquireLock(_this8.lockAcquireTimeout, async () => {
	...
	*/
	async signInWithWeb3(credentials) {
		...
		switch (chain) {
			case "ethereum": return await _this9.signInWithEthereum(credentials);
			case "solana": return await _this9.signInWithSolana(credentials);
	...
	async signInWithEthereum(credentials) {
		...
		try {
			const { data, error } = await _request(_this10.fetch, "POST", `${_this10.url}/token?grant_type=web3`, {
			...
			if (data.session) {
				await _this10._saveSession(data.session);
				await _this10._notifyAllSubscribers("SIGNED_IN", data.session);
			}
			return _this10._returnResult({
				data: Object.assign({}, data),
				error
	...
	async signInWithSolana(credentials) {
		...
		try {
			const { data, error } = await _request(_this11.fetch, "POST", `${_this11.url}/token?grant_type=web3`, {
				headers: _this11.headers,
				body: Object.assign({
					chain: "solana",
					message,
			...
			if (data.session) {
				await _this11._saveSession(data.session);
				await _this11._notifyAllSubscribers("SIGNED_IN", data.session);
			}
			return _this11._returnResult({
				data: Object.assign({}, data),
				error
	...
	async _exchangeCodeForSession(authCode) {
		var _this12 = this;
		const storageItem = await getItemAsync(_this12.storage, `${_this12.storageKey}-code-verifier`);
		const [codeVerifier, redirectType] = (storageItem !== null && storageItem !== void 0 ? storageItem : "").split("/");
		try {
			if (!codeVerifier && _this12.flowType === "pkce") throw new AuthPKCECodeVerifierMissingError();
			const { data, error } = await _request(_this12.fetch, "POST", `${_this12.url}/token?grant_type=pkce`, {
				headers: _this12.headers,
				body: {
					auth_code: authCode,
			...
			await removeItemAsync(_this12.storage, `${_this12.storageKey}-code-verifier`);
			if (error) throw error;
			if (!data || !data.session || !data.user) {
				const invalidTokenError = new AuthInvalidTokenResponseError();
			...
			if (data.session) {
				await _this12._saveSession(data.session);
				await _this12._notifyAllSubscribers(redirectType === "recovery" ? "PASSWORD_RECOVERY" : "SIGNED_IN", data.session);
			}
			return _this12._returnResult({
	...
	*/
	async signInWithIdToken(credentials) {
		...
		try {
			...
			const { data, error } = await _request(_this13.fetch, "POST", `${_this13.url}/token?grant_type=id_token`, {
				headers: _this13.headers,
				body: {
					provider,
					id_token: token,
			...
			if (data.session) {
				await _this13._saveSession(data.session);
				await _this13._notifyAllSubscribers("SIGNED_IN", data.session);
			}
			return _this13._returnResult({
				data,
				error
	...
	async signInWithOtp(credentials) {
		...
		try {
			if ("email" in credentials) {
				...
				let codeChallenge = null;
				let codeChallengeMethod = null;
				if (_this14.flowType === "pkce") [codeChallenge, codeChallengeMethod] = await getCodeChallengeAndMethod(_this14.storage, _this14.storageKey);
				const { error } = await _request(_this14.fetch, "POST", `${_this14.url}/otp`, {
					headers: _this14.headers,
					body: {
						email,
						data: (_a = options === null || options === void 0 ? void 0 : options.data) !== null && _a !== void 0 ? _a : {},
			...
			if ("phone" in credentials) {
				...
				const { data, error } = await _request(_this14.fetch, "POST", `${_this14.url}/otp`, {
					headers: _this14.headers,
					body: {
						phone,
			...
			await removeItemAsync(_this14.storage, `${_this14.storageKey}-code-verifier`);
	...
	*/
	async verifyOtp(params) {
		...
		try {
			...
			const { data, error } = await _request(_this15.fetch, "POST", `${_this15.url}/verify`, {
				headers: _this15.headers,
			...
			if (session === null || session === void 0 ? void 0 : session.access_token) {
				await _this15._saveSession(session);
				await _this15._notifyAllSubscribers(params.type == "recovery" ? "PASSWORD_RECOVERY" : "SIGNED_IN", session);
			}
	...
	*/
	async signInWithSSO(params) {
		...
		try {
			let codeChallenge = null;
			let codeChallengeMethod = null;
			if (_this16.flowType === "pkce") [codeChallenge, codeChallengeMethod] = await getCodeChallengeAndMethod(_this16.storage, _this16.storageKey);
			const result = await _request(_this16.fetch, "POST", `${_this16.url}/sso`, {
			...
			await removeItemAsync(_this16.storage, `${_this16.storageKey}-code-verifier`);
	...
	*/
	async reauthenticate() {
		var _this17 = this;
		await _this17.initializePromise;
		if (_this17.lock != null) return await _this17._acquireLock(_this17.lockAcquireTimeout, async () => {
			return await _this17._reauthenticate();
		});
		return await _this17._reauthenticate();
	}
	async _reauthenticate() {
		var _this18 = this;
		try {
			return await _this18._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				if (sessionError) throw sessionError;
				if (!session) throw new AuthSessionMissingError();
				const { error } = await _request(_this18.fetch, "GET", `${_this18.url}/reauthenticate`, {
					headers: _this18.headers,
	...
	async resend(credentials) {
		...
		try {
			...
			if ("email" in credentials) {
				...
				let codeChallenge = null;
				let codeChallengeMethod = null;
				if (_this19.flowType === "pkce") [codeChallenge, codeChallengeMethod] = await getCodeChallengeAndMethod(_this19.storage, _this19.storageKey);
				const { error } = await _request(_this19.fetch, "POST", endpoint, {
					headers: _this19.headers,
					body: {
						email,
						type,
				...
				if (error) await removeItemAsync(_this19.storage, `${_this19.storageKey}-code-verifier`);
				...
				const { data, error } = await _request(_this19.fetch, "POST", endpoint, {
					headers: _this19.headers,
					body: {
						phone,
						type,
			...
			await removeItemAsync(_this19.storage, `${_this19.storageKey}-code-verifier`);
	...
	*/
	async getSession() {
		var _this20 = this;
		await _this20.initializePromise;
		if (_this20.lock != null) return await _this20._acquireLock(_this20.lockAcquireTimeout, async () => {
			return _this20._useSession(async (result) => {
				return result;
			});
		});
		return await _this20._useSession(async (result) => {
			return result;
		});
	}
	/**
	...
	async _acquireLock(acquireTimeout, fn) {
		...
		try {
			if (_this21.lockAcquired) {
				...
				_this21.pendingInLock.push((async () => {
					try {
						await result;
					} catch (_e) {}
				})());
			...
			return await _this21.lock(`lock:${_this21.storageKey}`, acquireTimeout, async () => {
	...
	*/
	async _useSession(fn) {
		...
		try {
			return await fn(await _this22.__loadSession());
		} finally {
	...
	*/
	async __loadSession() {
		...
		try {
			...
			const maybeSession = await getItemAsync(_this23.storage, _this23.storageKey);
			...
			if (!hasExpired) {
				if (_this23.userStorage) {
					const maybeUser = await getItemAsync(_this23.userStorage, _this23.storageKey + "-user");
			...
			const { data: session, error } = await _this23._callRefreshToken(currentSession.refresh_token);
			if (error) {
				if (!!(currentSession.expires_at && currentSession.expires_at * 1e3 > Date.now())) {
					const stillStored = await getItemAsync(_this23.storage, _this23.storageKey);
	...
	*/
	async getUser(jwt) {
		var _this24 = this;
		if (jwt) return await _this24._getUser(jwt);
		await _this24.initializePromise;
		let result;
		if (_this24.lock != null) result = await _this24._acquireLock(_this24.lockAcquireTimeout, async () => {
			return await _this24._getUser();
		});
		else result = await _this24._getUser();
	...
	async _getUser(jwt) {
		var _this25 = this;
		try {
			if (jwt) return await _request(_this25.fetch, "GET", `${_this25.url}/user`, {
				headers: _this25.headers,
				jwt,
				xform: _userResponse
			});
			return await _this25._useSession(async (result) => {
				var _a, _b, _c;
				...
				return await _request(_this25.fetch, "GET", `${_this25.url}/user`, {
					headers: _this25.headers,
			...
			if (isAuthError(error)) {
				if (isAuthSessionMissingError(error)) {
					await _this25._removeSession();
					await removeItemAsync(_this25.storage, `${_this25.storageKey}-code-verifier`);
				}
	...
	*/
	async updateUser(attributes, options = {}) {
		var _this26 = this;
		await _this26.initializePromise;
		if (_this26.lock != null) return await _this26._acquireLock(_this26.lockAcquireTimeout, async () => {
			return await _this26._updateUser(attributes, options);
		});
		return await _this26._updateUser(attributes, options);
	}
	async _updateUser(attributes, options = {}) {
		var _this27 = this;
		try {
			return await _this27._useSession(async (result) => {
				...
				let codeChallenge = null;
				let codeChallengeMethod = null;
				if (_this27.flowType === "pkce" && attributes.email != null) [codeChallenge, codeChallengeMethod] = await getCodeChallengeAndMethod(_this27.storage, _this27.storageKey);
				const { data, error: userError } = await _request(_this27.fetch, "PUT", `${_this27.url}/user`, {
				...
				await _this27._saveSession(session);
				await _this27._notifyAllSubscribers("USER_UPDATED", session);
			...
			await removeItemAsync(_this27.storage, `${_this27.storageKey}-code-verifier`);
	...
	*/
	async setSession(currentSession) {
		var _this28 = this;
		await _this28.initializePromise;
		if (_this28.lock != null) return await _this28._acquireLock(_this28.lockAcquireTimeout, async () => {
			return await _this28._setSession(currentSession);
		});
		return await _this28._setSession(currentSession);
	}
	async _setSession(currentSession) {
		...
		try {
			...
			if (hasExpired) {
				const { data: refreshedSession, error } = await _this29._callRefreshToken(currentSession.refresh_token);
				...
				const { data, error } = await _this29._getUser(currentSession.access_token);
				if (error) return _this29._returnResult({
					data: {
						user: null,
						session: null
				...
				await _this29._saveSession(session);
				await _this29._notifyAllSubscribers("SIGNED_IN", session);
			}
	...
	*/
	async refreshSession(currentSession) {
		var _this30 = this;
		await _this30.initializePromise;
		if (_this30.lock != null) return await _this30._acquireLock(_this30.lockAcquireTimeout, async () => {
			return await _this30._refreshSession(currentSession);
		});
		return await _this30._refreshSession(currentSession);
	}
	async _refreshSession(currentSession) {
		var _this31 = this;
		try {
			return await _this31._useSession(async (result) => {
				var _a;
				if (!currentSession) {
					const { data, error } = result;
					if (error) throw error;
				...
				const { data: session, error } = await _this31._callRefreshToken(currentSession.refresh_token);
	...
	async _getSessionFromURL(params, callbackUrlType) {
		...
		try {
			...
			if (callbackUrlType === "pkce") {
				...
				const { data, error } = await _this32._exchangeCodeForSession(params.code);
				if (error) throw error;
			...
			const { data, error } = await _this32._getUser(access_token);
			if (error) throw error;
			const session = {
				provider_token,
	...
	*/
	async _isPKCECallback(params) {
		var _this33 = this;
		const currentStorageContent = await getItemAsync(_this33.storage, `${_this33.storageKey}-code-verifier`);
	...
	*/
	async signOut(options = { scope: "global" }) {
		var _this34 = this;
		await _this34.initializePromise;
		if (_this34.lock != null) return await _this34._acquireLock(_this34.lockAcquireTimeout, async () => {
			return await _this34._signOut(options);
		});
		return await _this34._signOut(options);
	}
	async _signOut({ scope } = { scope: "global" }) {
		var _this35 = this;
		return await _this35._useSession(async (result) => {
			var _a;
			...
			if (accessToken) {
				const { error } = await _this35.admin.signOut(accessToken, scope);
				if (error) {
					if (!(isAuthApiError(error) && (error.status === 404 || error.status === 401 || error.status === 403) || isAuthSessionMissingError(error))) return _this35._returnResult({ error });
				}
			}
			if (scope !== "others") {
				await _this35._removeSession();
				await removeItemAsync(_this35.storage, `${_this35.storageKey}-code-verifier`);
			}
	...
	*/
	onAuthStateChange(callback) {
		...
		(async () => {
			await _this36.initializePromise;
			if (_this36.lock != null) await _this36._acquireLock(_this36.lockAcquireTimeout, async () => {
				_this36._emitInitialSession(id);
			});
			else await _this36._emitInitialSession(id);
		})();
		return { data: { subscription } };
	}
	async _emitInitialSession(id) {
		var _this37 = this;
		return await _this37._useSession(async (result) => {
			var _a, _b;
			try {
				const { data: { session }, error } = result;
				if (error) throw error;
				await ((_a = _this37.stateChangeEmitters.get(id)) === null || _a === void 0 ? void 0 : _a.callback("INITIAL_SESSION", session));
				_this37._debug("INITIAL_SESSION", "callback id", id, "session", session);
			} catch (err) {
				await ((_b = _this37.stateChangeEmitters.get(id)) === null || _b === void 0 ? void 0 : _b.callback("INITIAL_SESSION", null));
	...
	*/
	async resetPasswordForEmail(email, options = {}) {
		...
		let codeChallenge = null;
		let codeChallengeMethod = null;
		if (_this38.flowType === "pkce") [codeChallenge, codeChallengeMethod] = await getCodeChallengeAndMethod(_this38.storage, _this38.storageKey, true);
		try {
			return await _request(_this38.fetch, "POST", `${_this38.url}/recover`, {
				body: {
					email,
					code_challenge: codeChallenge,
					code_challenge_method: codeChallengeMethod,
			...
			await removeItemAsync(_this38.storage, `${_this38.storageKey}-code-verifier`);
	...
	*/
	async getUserIdentities() {
		...
		var _a;
		try {
			const { data, error } = await _this39.getUser();
			if (error) throw error;
	...
	*/
	...
	async linkIdentityOAuth(credentials) {
		...
		var _a;
		try {
			const { data, error } = await _this41._useSession(async (result) => {
				var _a, _b, _c, _d, _f;
				const { data, error } = result;
				if (error) throw error;
				const url = await _this41._getUrlForProvider(`${_this41.url}/user/identities/authorize`, credentials.provider, {
					redirectTo: (_a = credentials.options) === null || _a === void 0 ? void 0 : _a.redirectTo,
					scopes: (_b = credentials.options) === null || _b === void 0 ? void 0 : _b.scopes,
					queryParams: (_c = credentials.options) === null || _c === void 0 ? void 0 : _c.queryParams,
					skipBrowserRedirect: true
				});
				return await _request(_this41.fetch, "GET", url, {
	...
	async linkIdentityIdToken(credentials) {
		var _this42 = this;
		return await _this42._useSession(async (result) => {
			var _a;
			try {
				const { error: sessionError, data: { session } } = result;
				if (sessionError) throw sessionError;
				const { options, provider, token, access_token, nonce } = credentials;
				const { data, error } = await _request(_this42.fetch, "POST", `${_this42.url}/token?grant_type=id_token`, {
				...
				if (data.session) {
					await _this42._saveSession(data.session);
					await _this42._notifyAllSubscribers("USER_UPDATED", data.session);
				}
				...
				await removeItemAsync(_this42.storage, `${_this42.storageKey}-code-verifier`);
	...
	*/
	async unlinkIdentity(identity) {
		var _this43 = this;
		try {
			return await _this43._useSession(async (result) => {
				var _a, _b;
				const { data, error } = result;
				if (error) throw error;
				return await _request(_this43.fetch, "DELETE", `${_this43.url}/user/identities/${identity.identity_id}`, {
	...
	*/
	async _refreshAccessToken(refreshToken) {
		...
		try {
			...
			return await retryable(async (attempt) => {
				...
				return await _request(_this44.fetch, "POST", `${_this44.url}/token?grant_type=refresh_token`, {
	...
	async _handleProviderSignIn(provider, options) {
		var _this45 = this;
		const url = await _this45._getUrlForProvider(`${_this45.url}/authorize`, provider, {
			redirectTo: options.redirectTo,
	...
	*/
	async _recoverAndRefresh() {
		...
		try {
			const currentSession = await getItemAsync(_this46.storage, _this46.storageKey);
			if (currentSession && _this46.userStorage) {
				let maybeUser = await getItemAsync(_this46.userStorage, _this46.storageKey + "-user");
				if (!_this46.storage.isServer && Object.is(_this46.storage, _this46.userStorage) && !maybeUser) {
					maybeUser = { user: currentSession.user };
					await setItemAsync(_this46.userStorage, _this46.storageKey + "-user", maybeUser);
				}
				currentSession.user = (_a = maybeUser === null || maybeUser === void 0 ? void 0 : maybeUser.user) !== null && _a !== void 0 ? _a : userNotAvailableProxy();
			} else if (currentSession && !currentSession.user) {
				if (!currentSession.user) {
					const separateUser = await getItemAsync(_this46.storage, _this46.storageKey + "-user");
					if (separateUser && (separateUser === null || separateUser === void 0 ? void 0 : separateUser.user)) {
						currentSession.user = separateUser.user;
						await removeItemAsync(_this46.storage, _this46.storageKey + "-user");
						await setItemAsync(_this46.storage, _this46.storageKey, currentSession);
					} else currentSession.user = userNotAvailableProxy();
				}
			}
			...
			if (!_this46._isValidSession(currentSession)) {
				...
				if (currentSession !== null) await _this46._removeSession();
				return;
			}
			...
			if (expiresWithMargin) {
				if (_this46.autoRefreshToken && currentSession.refresh_token) {
					const { error } = await _this46._callRefreshToken(currentSession.refresh_token);
				...
				const { data, error: userError } = await _this46._getUser(currentSession.access_token);
				if (!userError && (data === null || data === void 0 ? void 0 : data.user)) {
					currentSession.user = data.user;
					await _this46._saveSession(currentSession);
					await _this46._notifyAllSubscribers("SIGNED_IN", currentSession);
			...
			else await _this46._notifyAllSubscribers("SIGNED_IN", currentSession);
		} catch (err) {
	...
	async _callRefreshToken(refreshToken) {
		...
		try {
			...
			const storedAtStart = await getItemAsync(_this47.storage, _this47.storageKey);
			const { data, error } = await _this47._refreshAccessToken(refreshToken);
			if (error) throw error;
			if (!data.session) throw new AuthSessionMissingError();
			const storedAfter = await getItemAsync(_this47.storage, _this47.storageKey);
			...
			await _this47._saveSession(data.session);
			if (_this47._sessionRemovalEpoch !== epochBeforeSave) {
				...
				await removeItemAsync(_this47.storage, _this47.storageKey);
				if (_this47.userStorage) await removeItemAsync(_this47.userStorage, _this47.storageKey + "-user");
				const discarded = {
					data: null,
					error: new AuthRefreshDiscardedError()
				};
			...
			await _this47._notifyAllSubscribers("TOKEN_REFRESHED", data.session);
			const result = {
				data: data.session,
				error: null
			};
			...
			if (isAuthError(error)) {
				...
				if (!isAuthRetryableFetchError(error)) {
					const storedNow = await getItemAsync(_this47.storage, _this47.storageKey);
					...
					else await _this47._removeSession();
				}
	...
	async _notifyAllSubscribers(event, session, broadcast = true) {
		...
		try {
			...
			const errors = [];
			const promises = Array.from(_this48.stateChangeEmitters.values()).map(async (x) => {
				try {
	...
	*/
	async _saveSession(session) {
		...
		await removeItemAsync(_this49.storage, `${_this49.storageKey}-code-verifier`);
		const sessionToProcess = Object.assign({}, session);
		const userIsProxy = sessionToProcess.user && sessionToProcess.user.__isUserNotAvailableProxy === true;
		if (_this49.userStorage) {
			if (!userIsProxy && sessionToProcess.user) await setItemAsync(_this49.userStorage, _this49.storageKey + "-user", { user: sessionToProcess.user });
			else if (userIsProxy) {}
			const mainSessionData = Object.assign({}, sessionToProcess);
			delete mainSessionData.user;
			const clonedMainSessionData = deepClone(mainSessionData);
			await setItemAsync(_this49.storage, _this49.storageKey, clonedMainSessionData);
		} else {
			const clonedSession = deepClone(sessionToProcess);
			await setItemAsync(_this49.storage, _this49.storageKey, clonedSession);
		}
	}
	async _removeSession() {
		...
		await removeItemAsync(_this50.storage, _this50.storageKey);
		await removeItemAsync(_this50.storage, _this50.storageKey + "-code-verifier");
		await removeItemAsync(_this50.storage, _this50.storageKey + "-user");
		if (_this50.userStorage) await removeItemAsync(_this50.userStorage, _this50.storageKey + "-user");
		await _this50._notifyAllSubscribers("SIGNED_OUT", null);
	}
	/**
	...
	*/
	async _startAutoRefresh() {
		var _this51 = this;
		await _this51._stopAutoRefresh();
		...
		const timeout = setTimeout(async () => {
			await _this51.initializePromise;
			await _this51._autoRefreshTokenTick();
		}, 0);
	...
	*/
	async startAutoRefresh() {
		...
		await _this53._startAutoRefresh();
	}
	/**
	...
	*/
	async stopAutoRefresh() {
		...
		await _this54._stopAutoRefresh();
	}
	/**
	...
	*/
	async dispose() {
		...
		await _this55._stopAutoRefresh();
	...
	*/
	async _autoRefreshTokenTick() {
		...
		if (_this56.lock != null) {
			try {
				await _this56._acquireLock(0, async () => {
					try {
						const now = Date.now();
						try {
							return await _this56._useSession(async (result) => {
								...
								if (expiresInTicks <= 3) await _this56._callRefreshToken(session.refresh_token);
							});
						} catch (e) {
		...
		try {
			const now = Date.now();
			try {
				await _this56._useSession(async (result) => {
					const { data: { session } } = result;
					if (!session || !session.refresh_token || !session.expires_at) {
						_this56._debug("#_autoRefreshTokenTick()", "no session");
						return;
	...
	*/
	async _handleVisibilityChange() {
		...
		try {
			_this57.visibilityChangedCallback = async () => {
				try {
					await _this57._onVisibilityChanged(false);
				} catch (error) {
			...
			await _this57._onVisibilityChanged(true);
		} catch (error) {
	...
	async _onVisibilityChanged(calledFromInitialize) {
		...
		if (document.visibilityState === "visible") {
			...
			if (!calledFromInitialize) {
				await _this58.initializePromise;
				if (_this58.lock != null) await _this58._acquireLock(_this58.lockAcquireTimeout, async () => {
					if (document.visibilityState !== "visible") {
					...
					await _this58._recoverAndRefresh();
				});
				else {
					if (document.visibilityState !== "visible") {
	...
	*/
	async _getUrlForProvider(url, provider, options) {
		...
		if (_this59.flowType === "pkce") {
			const [codeChallenge, codeChallengeMethod] = await getCodeChallengeAndMethod(_this59.storage, _this59.storageKey);
	...
	async _unenroll(params) {
		var _this60 = this;
		try {
			return await _this60._useSession(async (result) => {
				var _a;
				const { data: sessionData, error: sessionError } = result;
				if (sessionError) return _this60._returnResult({
					data: null,
					error: sessionError
				});
				return await _request(_this60.fetch, "DELETE", `${_this60.url}/factors/${params.factorId}`, {
					headers: _this60.headers,
					jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
				});
			});
		} catch (error) {
			if (isAuthError(error)) return _this60._returnResult({
				data: null,
				error
			});
			throw error;
	...
	async _enroll(params) {
		var _this61 = this;
		try {
			return await _this61._useSession(async (result) => {
				var _a, _b;
				const { data: sessionData, error: sessionError } = result;
				...
				const { data, error } = await _request(_this61.fetch, "POST", `${_this61.url}/factors`, {
					body,
					headers: _this61.headers,
					jwt: (_a = sessionData === null || sessionData === void 0 ? void 0 : sessionData.session) === null || _a === void 0 ? void 0 : _a.access_token
				});
				if (error) return _this61._returnResult({
					data: null,
					error
				});
	...
	async _verify(params) {
		...
		const run = async () => {
			try {
				return await _this62._useSession(async (result) => {
					var _a;
					const { data: sessionData, error: sessionError } = result;
					...
					const { data, error } = await _request(_this62.fetch, "POST", `${_this62.url}/factors/${params.factorId}/verify`, {
						body,
					...
					await _this62._saveSession(Object.assign({ expires_at: Math.round(Date.now() / 1e3) + data.expires_in }, data));
					await _this62._notifyAllSubscribers("MFA_CHALLENGE_VERIFIED", data);
		...
		if (_this62.lock != null) return _this62._acquireLock(_this62.lockAcquireTimeout, run);
		return run();
	}
	async _challenge(params) {
		...
		const run = async () => {
			try {
				return await _this63._useSession(async (result) => {
					var _a;
					const { data: sessionData, error: sessionError } = result;
					...
					const response = await _request(_this63.fetch, "POST", `${_this63.url}/factors/${params.factorId}/challenge`, {
						body: params,
	...
	*/
	async _challengeAndVerify(params) {
		var _this64 = this;
		const { data: challengeData, error: challengeError } = await _this64._challenge({ factorId: params.factorId });
		if (challengeError) return _this64._returnResult({
			data: null,
			error: challengeError
		});
		return await _this64._verify({
	...
	*/
	async _listFactors() {
		var _this65 = this;
		var _a;
		const { data: { user }, error: userError } = await _this65.getUser();
		if (userError) return {
			data: null,
			error: userError
		};
	...
	*/
	async _getAuthenticatorAssuranceLevel(jwt) {
		...
		if (jwt) try {
			...
			const { data: { user }, error: userError } = await _this66.getUser(jwt);
			if (userError) return _this66._returnResult({
				data: null,
				error: userError
			});
			...
			if (isAuthError(error)) return _this66._returnResult({
				data: null,
				error
			});
			throw error;
		}
		const { data: { session }, error: sessionError } = await _this66.getSession();
		if (sessionError) return _this66._returnResult({
			data: null,
			error: sessionError
		});
		if (!session) return {
	...
	*/
	async _getAuthorizationDetails(authorizationId) {
		var _this67 = this;
		try {
			return await _this67._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				if (sessionError) return _this67._returnResult({
					data: null,
					error: sessionError
				});
				if (!session) return _this67._returnResult({
					data: null,
					error: new AuthSessionMissingError()
				});
				return await _request(_this67.fetch, "GET", `${_this67.url}/oauth/authorizations/${authorizationId}`, {
					headers: _this67.headers,
					jwt: session.access_token,
					xform: (data) => ({
						data,
						error: null
	...
	*/
	async _approveAuthorization(authorizationId, options) {
		var _this68 = this;
		try {
			return await _this68._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				if (sessionError) return _this68._returnResult({
					data: null,
					error: sessionError
				});
				if (!session) return _this68._returnResult({
					data: null,
					error: new AuthSessionMissingError()
				});
				const response = await _request(_this68.fetch, "POST", `${_this68.url}/oauth/authorizations/${authorizationId}/consent`, {
	...
	*/
	async _denyAuthorization(authorizationId, options) {
		var _this69 = this;
		try {
			return await _this69._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				if (sessionError) return _this69._returnResult({
					data: null,
					error: sessionError
				});
				if (!session) return _this69._returnResult({
					data: null,
					error: new AuthSessionMissingError()
				});
				const response = await _request(_this69.fetch, "POST", `${_this69.url}/oauth/authorizations/${authorizationId}/consent`, {
	...
	*/
	async _listOAuthGrants() {
		var _this70 = this;
		try {
			return await _this70._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				if (sessionError) return _this70._returnResult({
					data: null,
					error: sessionError
				});
				if (!session) return _this70._returnResult({
					data: null,
					error: new AuthSessionMissingError()
				});
				return await _request(_this70.fetch, "GET", `${_this70.url}/user/oauth/grants`, {
	...
	*/
	async _revokeOAuthGrant(options) {
		var _this71 = this;
		try {
			return await _this71._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				if (sessionError) return _this71._returnResult({
					data: null,
					error: sessionError
				});
				if (!session) return _this71._returnResult({
					data: null,
					error: new AuthSessionMissingError()
				});
				await _request(_this71.fetch, "DELETE", `${_this71.url}/user/oauth/grants`, {
	...
	async fetchJwk(kid, jwks = { keys: [] }) {
		...
		if (jwk) return jwk;
		const now = Date.now();
		jwk = _this72.jwks.keys.find((key) => key.kid === kid);
		if (jwk && _this72.jwks_cached_at + 6e5 > now) return jwk;
		const { data, error } = await _request(_this72.fetch, "GET", `${_this72.url}/.well-known/jwks.json`, { headers: _this72.headers });
		if (error) throw error;
		if (!data.keys || data.keys.length === 0) return null;
		_this72.jwks = data;
		_this72.jwks_cached_at = now;
	...
	*/
	async getClaims(jwt, options = {}) {
		...
		try {
			let token = jwt;
			if (!token) {
				const { data, error } = await _this73.getSession();
				if (error || !data.session) return _this73._returnResult({
					data: null,
					error
				});
			...
			const signingKey = !header.alg || header.alg.startsWith("HS") || !header.kid || !("crypto" in globalThis && "subtle" in globalThis.crypto) ? null : await _this73.fetchJwk(header.kid, (options === null || options === void 0 ? void 0 : options.keys) ? { keys: options.keys } : options === null || options === void 0 ? void 0 : options.jwks);
			if (!signingKey) {
				const { error } = await _this73.getUser(token);
				if (error) throw error;
				return {
					data: {
						claims: payload,
	...
	*/
	async signInWithPasskey(credentials) {
		var _this74 = this;
		var _a, _b, _c;
		assertPasskeyExperimentalEnabled(_this74.experimental);
		try {
			if (!browserSupportsWebAuthn()) return _this74._returnResult({
				data: null,
				error: new AuthUnknownError("Browser does not support WebAuthn", null)
			});
			const { data: options, error: optionsError } = await _this74._startPasskeyAuthentication({ options: { captchaToken: (_a = credentials === null || credentials === void 0 ? void 0 : credentials.options) === null || _a === void 0 ? void 0 : _a.captchaToken } });
			if (optionsError || !options) return _this74._returnResult({
				data: null,
				error: optionsError
			});
			...
			if (credentialError || !credential) return _this74._returnResult({
				data: null,
			...
			return _this74._verifyPasskeyAuthentication({
	...
	*/
	async registerPasskey(credentials) {
		var _this75 = this;
		var _a, _b;
		assertPasskeyExperimentalEnabled(_this75.experimental);
		try {
			if (!browserSupportsWebAuthn()) return _this75._returnResult({
				data: null,
				error: new AuthUnknownError("Browser does not support WebAuthn", null)
			});
			const { data: options, error: optionsError } = await _this75._startPasskeyRegistration();
			if (optionsError || !options) return _this75._returnResult({
				data: null,
				error: optionsError
			});
			...
			if (credentialError || !credential) return _this75._returnResult({
				data: null,
				error: credentialError !== null && credentialError !== void 0 ? credentialError : new AuthUnknownError("WebAuthn ceremony failed", null)
			});
			const serialized = serializeCredentialCreationResponse(credential);
			return _this75._verifyPasskeyRegistration({
				challengeId: options.challenge_id,
				credential: serialized
			});
		} catch (error) {
	...
	*/
	async _startPasskeyRegistration() {
		var _this76 = this;
		assertPasskeyExperimentalEnabled(_this76.experimental);
		try {
			return await _this76._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				...
				const { data, error } = await _request(_this76.fetch, "POST", `${_this76.url}/passkeys/registration/options`, {
					headers: _this76.headers,
					jwt: session.access_token,
					body: {}
				});
				if (error) return _this76._returnResult({
					data: null,
					error
				});
				return _this76._returnResult({
					data,
					error: null
				});
			});
	...
	*/
	async _verifyPasskeyRegistration(params) {
		var _this77 = this;
		assertPasskeyExperimentalEnabled(_this77.experimental);
		try {
			return await _this77._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				...
				const { data, error } = await _request(_this77.fetch, "POST", `${_this77.url}/passkeys/registration/verify`, {
				...
				if (error) return _this77._returnResult({
					data: null,
					error
				});
				return _this77._returnResult({
					data,
					error: null
				});
			});
	...
	*/
	async _startPasskeyAuthentication(params) {
		var _this78 = this;
		var _a;
		assertPasskeyExperimentalEnabled(_this78.experimental);
		try {
			const { data, error } = await _request(_this78.fetch, "POST", `${_this78.url}/passkeys/authentication/options`, {
				headers: _this78.headers,
			...
			if (error) return _this78._returnResult({
				data: null,
				error
			});
			return _this78._returnResult({
				data,
				error: null
			});
		} catch (error) {
			if (isAuthError(error)) return _this78._returnResult({
				data: null,
				error
			});
			throw error;
	...
	*/
	async _verifyPasskeyAuthentication(params) {
		var _this79 = this;
		assertPasskeyExperimentalEnabled(_this79.experimental);
		try {
			const { data, error } = await _request(_this79.fetch, "POST", `${_this79.url}/passkeys/authentication/verify`, {
				headers: _this79.headers,
				body: {
					challenge_id: params.challengeId,
					credential: params.credential
				},
			...
			if (error) return _this79._returnResult({
				data: null,
				error
			});
			if (data.session) {
				await _this79._saveSession(data.session);
				await _this79._notifyAllSubscribers("SIGNED_IN", data.session);
			}
			return _this79._returnResult({
				data,
				error: null
			});
		} catch (error) {
	...
	*/
	async _listPasskeys() {
		...
		try {
			return await _this80._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				if (sessionError) return _this80._returnResult({
					data: null,
					error: sessionError
				});
				if (!session) return _this80._returnResult({
					data: null,
					error: new AuthSessionMissingError()
				});
				const { data, error } = await _request(_this80.fetch, "GET", `${_this80.url}/passkeys`, {
					headers: _this80.headers,
					jwt: session.access_token,
					xform: (data) => ({
						data,
						error: null
				...
				if (error) return _this80._returnResult({
					data: null,
					error
				});
				return _this80._returnResult({
					data,
					error: null
				});
			});
	...
	*/
	async _updatePasskey(params) {
		...
		try {
			return await _this81._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				if (sessionError) return _this81._returnResult({
					data: null,
					error: sessionError
				});
				if (!session) return _this81._returnResult({
					data: null,
					error: new AuthSessionMissingError()
				});
				const { data, error } = await _request(_this81.fetch, "PATCH", `${_this81.url}/passkeys/${params.passkeyId}`, {
					headers: _this81.headers,
					jwt: session.access_token,
					body: { friendly_name: params.friendlyName }
				});
				if (error) return _this81._returnResult({
					data: null,
					error
				});
				return _this81._returnResult({
					data,
					error: null
				});
			});
	...
	*/
	async _deletePasskey(params) {
		...
		try {
			return await _this82._useSession(async (result) => {
				const { data: { session }, error: sessionError } = result;
				...
				const { error } = await _request(_this82.fetch, "DELETE", `${_this82.url}/passkeys/${params.passkeyId}`, {
