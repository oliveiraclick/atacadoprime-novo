...
var RealtimeChannel = class RealtimeChannel {
	...
	*/
	async httpSend(event, payload, opts = {}) {
		...
		const response = await _this4._fetchWithTimeout(url.toString(), options, (_a = opts.timeout) !== null && _a !== void 0 ? _a : _this4.timeout);
	...
	*/
	async send(args, opts = {}) {
		...
		if (!_this5.channelAdapter.canPush() && args.type === "broadcast") {
			...
			try {
				const response = await _this5._fetchWithTimeout(_this5.broadcastEndpointURL, options, (_a = opts.timeout) !== null && _a !== void 0 ? _a : _this5.timeout);
	...
	*/
	...
	async _fetchWithTimeout(url, options, timeout) {
		...
		const response = await _this7.socket.fetch(url, Object.assign(Object.assign({}, options), { signal: controller.signal }));
		clearTimeout(id);
		return response;
	}
	/** @internal */
  ...
  const client = new RealtimeClient(url, {
	...
	*/
	async disconnect(code, reason) {
		...
		return await _this.socketAdapter.disconnect(() => {
	...
	*/
	async removeAllChannels() {
		var _this2 = this;
		const promises = _this2.channels.map(async (channel) => {
			const result = await channel.unsubscribe();
			channel.teardown();
			return result;
		});
		const result = await Promise.all(promises);
		await _this2.disconnect();
		return result;
	}
	/**
	* Logs the message.
	...
	*/
	async _performAuth(token = null) {
		...
		if (token) {
			...
			tokenToSend = await _this5.accessToken();
		} catch (e) {
		...
		if (isManualToken) _this5._manuallySetToken = true;
		else if (_this5.accessToken) _this5._manuallySetToken = false;
	...
	*/
	async _waitForAuthIfNeeded() {
		var _this6 = this;
		if (_this6._authPromise) await _this6._authPromise;
	}
	/**
	...
	*/
	...
	async _reconnectAuth() {
		var _this7 = this;
		await _this7._waitForAuthIfNeeded();
