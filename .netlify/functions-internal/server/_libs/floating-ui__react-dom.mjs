...
import { d as _objectSpread2, f as init_objectSpread2, l as require_react_dom } from "./@dnd-kit/core.mjs";
...
function useFloating(options) {
	...
	const update = import_react.useCallback(() => {
		...
		computePosition(referenceRef.current, floatingRef.current, config).then((data) => {
			const fullData = _objectSpread2(_objectSpread2({}, data), {}, { isPositioned: openRef.current !== false });
	...
	index(() => {
		if (open === false && dataRef.current.isPositioned) {
			...
			setData((data) => _objectSpread2(_objectSpread2({}, data), {}, { isPositioned: false }));
		}
	}, [open]);
	...
	const floatingStyles = import_react.useMemo(() => {
		...
		if (transform) return _objectSpread2(_objectSpread2({}, initialStyles), {}, { transform: "translate(" + x + "px, " + y + "px)" }, getDPR(elements.floating) >= 1.5 && { willChange: "transform" });
		return {
			position: strategy,
			left: x,
			top: y
	...
	return import_react.useMemo(() => _objectSpread2(_objectSpread2({}, data), {}, {
		update,
		refs,
		elements,
		floatingStyles
