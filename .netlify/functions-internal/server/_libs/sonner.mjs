	...
	constructor() {
		...
		this.create = (data) => {
			...
			if (alreadyExists) this.toasts = this.toasts.map((toast) => {
				if (toast.id === id) {
					this.publish(_objectSpread2(_objectSpread2(_objectSpread2({}, toast), data), {}, {
						id,
						title: message
					}));
			...
			else this.addToast(_objectSpread2(_objectSpread2({ title: message }, rest), {}, {
				dismissible,
				id
			}));
			return id;
		...
		this.message = (message, data) => {
			return this.create(_objectSpread2(_objectSpread2({}, data), {}, { message }));
		};
		...
		this.promise = (promise, data) => {
			if (!data) return;
			let id = void 0;
			if (data.loading !== void 0) id = this.create(_objectSpread2(_objectSpread2({}, data), {}, {
				promise,
				type: "loading",
...
var toastFunction = (message, data) => {
	const id = (data == null ? void 0 : data.id) || toastsCounter++;
	ToastState.addToast(_objectSpread2(_objectSpread2({ title: message }, data), {}, { id }));
	return id;
};
...
var Toast = (props) => {
	...
	import_react.useLayoutEffect(() => {
		...
		setHeights((heights) => {
			...
			else return heights.map((height) => height.toastId === toast.id ? _objectSpread2(_objectSpread2({}, height), {}, { height: newHeight }) : height);
		});
	}, [
		mounted,
		toast.title,
		...
		return ToastState.subscribe((toast) => {
			if (toast.dismiss) {
				requestAnimationFrame(() => {
					setToasts((toasts) => toasts.map((t) => t.id === toast.id ? _objectSpread2(_objectSpread2({}, t), {}, { delete: true }) : t));
				});
				return;
			}
			setTimeout(() => {
				import_react_dom.flushSync(() => {
					setToasts((toasts) => {
						...
						if (indexOfExistingToast !== -1) return [
							...
							_objectSpread2(_objectSpread2({}, toasts[indexOfExistingToast]), toast),
